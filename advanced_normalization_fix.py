...
logger = setup_logging("advanced_normalization_fix")
...
class AdvancedNormalizationProcessor:
    ...
    def __init__(self):
        ...
        self.corruption_patterns = [
            ...
            (
                r'style="[^"]*[^;]"[^>]*>',
                lambda m: self._fix_style_attribute(m.group(0)),
            ),
            # Remove orphaned closing tags
            (r"&lt;/[^&]+&gt;", ""),
            # Fix font-family corruption in style attributes
            (
                r"font-family:\s*'[^']*times[^']*'[^']*serif[^']*;",
        ...
        self.sentence_splitters = [
            ...
            (
                r"The evidence demonstrates ([^.!?]{25,}[.!?])",
                r"The evidence demonstrates \1",
            ),
            # Break long "Application Analysis" sentences
            (
                r"Application Analysis:\s*([^.!?]{30,}[.!?])",
                r"Application Analysis: \1",
            ),
        ]
    ...
    def _fix_style_attribute(self, style_attr: str) -> str:
        ...
        fixed_style = re.sub(
            r"font-family:\s*[^;]*times[^;]*;",
            'font-family: "Times New Roman", serif;',
            style_content,
        )
        fixed_style = re.sub(
            r"[^;]*\d+px[^;]*;", "", fixed_style
        )  # Remove malformed px declarations
    ...
    def process_html_content(self, html_content: str) -> str:
        ...
        logger.debug("🔧 ADVANCED NORMALIZATION: Starting HTML processing...")
        ...
        logger.debug("✅ ADVANCED NORMALIZATION: Processing complete")
        return final_content

    def _fix_html_corruption(self, content: str) -> str:
        """Fix HTML corruption patterns."""
        logger.info("🔧 Fixing HTML corruption...")
    ...
    def _normalize_text_nodes(self, soup: BeautifulSoup) -> None:
        ...
        logger.info("📝 Normalizing sentence length...")
    ...
    def _break_at_natural_points(self, sentence: str) -> List[str]:
        ...
        for i, word in enumerate(words):
            if (
                word.lower()
                in ["and", "but", "however", "moreover", "furthermore", "additionally"]
                and i > 5
                and i < len(words) - 5
            ):  # Not too close to beginning or end
