...
from backend_logic.utils.logging_config import get_module_logger, setup_logging
...
def start_analysis():
    ...
    logger.info(
        ...
        extra={
            "intake_form": intake_form.name
            if hasattr(intake_form, "name")
            else str(type(intake_form)),
    ...
    try:
        ...
        logger.exception(
            "Document analysis failed", extra={"error_type": type(e).__name__}
        )
        raise
    finally:
        loop.close()
