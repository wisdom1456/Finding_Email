...
from legal_portal.utils.tracing import Span, trace
...
def initialize_session_state():
    """Initialize the session state with default values including performance optimization settings."""
...
def start_analysis():
    """Handle the start analysis button click with comprehensive observability."""
    # Set request context
    ...
    app_logger.info("Analysis start requested", username=username, request_id=request_id)
    ...
    if not intake_form:
        app_logger.warning("Analysis start failed: no intake form provided", username=username)
    ...
    log_document_processing(
        document_name=intake_name, action="start_analysis", user=username, documents_count=len(case_documents)
    )
    ...
    try:
        ...
        log_performance_metric("document_analysis", processing_time, documents_count=len(case_documents))
    ...
    with tab1:
        ...
        with col1:
            ...
            if authentication_status:
                ...
                # Create user session
                user_data = {"name": name, "role": st.session_state.get("role", UserRole.VIEWER).value}
    ...
    with tab2:
        ...
        if available_providers:
            provider = st.selectbox(
                "Select SSO Provider", options=available_providers, format_func=lambda x: x.capitalize()
            )
...
def show_admin_panel():
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "role": st.column_config.SelectboxColumn(
                        "Role", options=["admin", "user", "viewer", "auditor"], required=True
                    ),
                },
            )
                ...
                if st.button("Update Role"):
                    if auth_manager.update_user_role(selected_user, new_role):
                        ...
                        session_monitor.log_role_changed(
                            selected_user, "unknown", new_role, st.session_state.get("username")
                        )
                        st.rerun()
            ...
            with col3:
                if st.button("Delete User", type="secondary"):
                    if auth_manager.delete_user(selected_user):
                        ...
                        session_monitor.log_user_deleted(selected_user, st.session_state.get("username"))
                        st.rerun()
    ...
    with tab2:
        ...
        if recent_activities:
            st.text_area("Recent Activities", value="".join(recent_activities), height=400, disabled=True)
        else:
    ...
    with tab3:
        ...
        timeout = st.number_input(
            "Session Timeout (minutes)", min_value=5, max_value=120, value=session_manager.timeout_minutes
        )

        max_sessions = st.number_input(
            "Max Sessions per User", min_value=1, max_value=10, value=session_manager.max_sessions_per_user
        )
...
def main():
    ...
    app_logger.info(
        "Legal Document Analysis Portal starting up",
    ...
    st.set_page_config(
        ...
        layout="wide",
        menu_items={"About": "Legal Document Analysis Portal v2.1 - Enterprise Edition with Authentication"},
    )
    ...
    if session_manager.check_session_timeout() and st.session_state.get("authentication_status"):
        ...
        # Log session timeout
        auth_logger.warning("Session timeout", username=username, session_id=session_id)

        # Audit log for compliance
        audit_logger.log_authentication(
            username=username, action="session_timeout", success=False, session_id=session_id
        )
    ...
    with tab1:
        if st.session_state.processing_status in ["idle", "failed", "completed"]:
            ...
            if (
                ...
                intake_form = st.session_state.get("intake_form")
                # case_documents = st.session_state.get("case_documents", [])  # Unused variable removed
