...
class TestAIAnalyzer:
    ...
    async def test_summarize_media_content(self, ai_analyzer, mock_openai_client):
        ...
        mock_openai_client.chat.completions.create.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content='{"summary": "Test summary"}'))]
        )
        summary = await ai_analyzer._summarize_media_content("test transcript", "audio", "test.mp3")
    ...
    async def test_integration_of_media_insights(self, ai_analyzer, mock_openai_client):
        ...
        ai_analyzer._make_openai_request = AsyncMock(side_effect=[
            {"summary": "Audio summary"},
            {"summary": "Video summary"},
            {
                "legal_assessment": LegalAssessment(claim_viability="Strong").model_dump(),
                "demand_letter_evaluation": DemandLetterEvaluation(is_appropriate=True).model_dump()
            }
        ])

        analysis = CaseAnalysisResult(
            intake_analysis=EnhancedIntakeAnalysis(client_name="Test"),
            transcripted_media=[TranscriptedMedia(file_name="test.mp3", transcript="long transcript")],
            video_insights=[VideoInsight(file_name="test.mp4", insights={"data": "video data"})]
        )

        result = await ai_analyzer.perform_final_assessment(analysis)

        assert "long transcript" not in ai_analyzer._make_openai_request.call_args_list[2][0][0]
        assert "Audio summary" in ai_analyzer._make_openai_request.call_args_list[2][0][0]
    ...
    async def test_handling_of_no_media_content(self, ai_analyzer):
        ...
        ai_analyzer._make_openai_request = AsyncMock(return_value={
             "legal_assessment": LegalAssessment(claim_viability="Strong").model_dump(),
             "demand_letter_evaluation": DemandLetterEvaluation(is_appropriate=True).model_dump()
        })
    ...
    async def test_token_management_with_media(self, ai_analyzer):
        ...
        with patch.object(ai_analyzer, '_truncate_content_if_needed', return_value="truncated") as mock_truncate:
