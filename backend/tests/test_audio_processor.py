...
from backend.utils.data_models import MediaProcessingError, TranscriptedMedia
...
class TestAudioProcessor:
    ...
    async def test_process_audio_file_success(
        self, mock_magic, audio_processor, mock_openai_client, temp_audio_file
    ):
        ...
        audio_processor._transcribe_with_whisper = AsyncMock(
    ...
    async def test_file_validation_size_limit(self, audio_processor, temp_audio_file):
        ...
        result = await audio_processor.process_audio_file(
            large_file_path, "large_file.mp3"
        )
    ...
    @pytest.mark.asyncio
    @patch("backend_logic.audio_processor.magic.from_file")
    async def test_file_validation_unsupported_format(
        self, mock_magic, audio_processor, temp_audio_file
    ):
        ...
        result = await audio_processor.process_audio_file(
            unsupported_file_path, "test.json"
        )
    ...
    @pytest.mark.asyncio
    @patch("backend_logic.audio_processor.magic.from_file")
    async def test_transcription_api_error_handling(
        self, mock_magic, audio_processor, mock_openai_client, temp_audio_file
    ):
        ...
        mock_openai_client.audio.transcriptions.create.side_effect = RateLimitError(
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_tempfile:
            ...
            with patch("os.path.exists") as mock_exists:
                ...
                with patch("os.unlink") as mock_unlink:
                    await audio_processor.process_audio_from_streamlit(
                        mock_uploaded_file, "test.mp3"
                    )
