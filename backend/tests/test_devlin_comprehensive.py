...
def categorize_documents() -> tuple[str, list[str]]:
    ...
    """
    print_banner(
        f"📁 DOCUMENT DISCOVERY - {CASE_REFERENCE} (TESTING MODE: 2 FILES ONLY)"
    )

    if not INPUT_PATH.exists():
        msg = f"Client documents directory not found: {INPUT_PATH}"
        raise FileNotFoundError(msg)
...
def validate_response(response: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        validation_results["checks"].append(
            {"check": "Document Intake", "status": "FAIL", "details": str(e)}
        )
    ...
    try:
        ...
        validation_results["checks"].append(
            {"check": "AI Analysis", "status": "FAIL", "details": str(e)}
        )
        validation_results["overall_status"] = "FAIL"
    ...
    try:
        ...
        validation_results["checks"].append(
            {"check": "File Generation", "status": "FAIL", "details": str(e)}
        )
        validation_results["overall_status"] = "FAIL"
    ...
    try:
        ...
        validation_results["checks"].append(
            {"check": "Email Comparison", "status": "FAIL", "details": str(e)}
        )
        validation_results["overall_status"] = "FAIL"
...
logger.debug('  - Validating Document Intake...')
    ...
    if not intake_analysis:
        ...
        if intake_analysis.get("client_name") != CLIENT_NAME:
            intake_check["status"] = "FAIL"
            intake_check["details"].append(
                f"Client name mismatch. Expected: {CLIENT_NAME}, Got: {intake_analysis.get('client_name')}"
            )
        if intake_analysis.get("case_type") != CASE_TYPE:
            intake_check["status"] = "FAIL"
            intake_check["details"].append(
                f"Case type mismatch. Expected: {CASE_TYPE}, Got: {intake_analysis.get('case_type')}"
            )
    ...
    try:
        ...
        if not generated_email_content:
            ...
            comparator = EmailComparator(
                str(reference_email_path), generated_email_content
            )
            ...
            if comparison_results.get("substance", {}).get("score", 0) < 0.8:
                ...
                comparison_check["details"].append(
                    f"Substance similarity score is low: {comparison_results.get('substance', {}).get('score', 0):.2f}"
                )
        ...
        comparison_check["status"] = "FAIL"
        comparison_check["details"].append(
            f"An error occurred during email comparison: {e}"
        )
