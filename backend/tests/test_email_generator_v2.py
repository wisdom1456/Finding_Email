...
from backend_logic.email_generator import EmailGeneratorV2, EmailGenerationError, SectionValidationError
...
def make_valid_devlin_analysis():
    return CaseAnalysisResult(
        ...
        legal_assessment=LegalAssessment(
            ...
            recommended_actions=["Send demand letter", "Prepare for possible litigation"],
...
def test_happy_path_devlin():
    ...
    for field in [
        ...
        assert field.startswith("<p>") or field.startswith("<h3>") or field.startswith("<")
...
def test_error_handling_fallback(monkeypatch):
    ...
    monkeypatch.setattr(generator, "_generate_factual_summary_content", lambda *a, **kw: "")
