...
async def test_end_to_end_with_mixed_media(mock_dependencies, mock_streamlit_files):
    ...
    mock_dependencies[
        "doc_processor"
    ].return_value.process_documents_from_streamlit.return_value = [
        ProcessedDocument(
    ...
    mock_dependencies[
        "audio_processor"
    ].return_value.process_audio_from_streamlit.return_value = TranscriptedMedia(
        file_name="meeting.mp3", transcript="audio transcript"
    )
    mock_dependencies[
        "video_processor"
    ].return_value.process_video_from_streamlit.return_value = VideoInsight(
        file_name="walkthrough.mp4", insights={"scenes": "video scenes"}
    )
    mock_dependencies[
        "ai_analyzer"
    ].return_value.analyze_intake.return_value = CaseAnalysisResult(
    ...
    with patch("streamlit.session_state", new_callable=MagicMock) as mock_session:
        ...
        assert (
            mock_dependencies[
                "ai_analyzer"
            ].return_value.perform_final_assessment.call_count
            == 1
        )
...
async def test_media_summarization_integration():
    ...
    ai_analyzer._make_openai_request = AsyncMock(
        return_value={"summary": "This is a media summary."}
    )

    analysis_result = CaseAnalysisResult(
        intake_analysis=EnhancedIntakeAnalysis(client_name="Test Client"),
        transcripted_media=[
            TranscriptedMedia(file_name="test.mp3", transcript="long audio transcript")
        ],
        video_insights=[
            VideoInsight(file_name="test.mp4", insights={"data": "long video insight"})
        ],
    )
...
async def test_email_generation_with_media_insights(mock_dependencies):
    ...
    final_analysis = CaseAnalysisResult(
        intake_analysis=EnhancedIntakeAnalysis(
            client_name="Test Client", case_type="Dispute"
        ),
        ...
        transcripted_media=[
            TranscriptedMedia(file_name="audio.mp3", transcript="Audio summary")
        ],
        video_insights=[VideoInsight(file_name="video.mp4", insights="Video summary")],
    )
    mock_dependencies[
        "ai_analyzer"
    ].return_value.perform_final_assessment.return_value = final_analysis
...
@pytest.mark.asyncio
async def test_error_recovery_with_media_failure(
    mock_dependencies, mock_streamlit_files
):
    ...
    mock_dependencies[
        "audio_processor"
    ].return_value.process_audio_from_streamlit.return_value = MediaProcessingError(
        file_name="bad_audio.mp3", error_message="processing failed"
    )
    ...
    with patch("streamlit.session_state", new_callable=MagicMock) as mock_session:
        ...
        final_assessment_call_args = mock_dependencies[
            "ai_analyzer"
        ].return_value.perform_final_assessment.call_args[0][0]
...
@pytest.mark.asyncio
async def test_large_file_handling_and_timeouts(
    mock_dependencies, mock_streamlit_files
):
    ...
    mock_dependencies[
        "video_processor"
    ].return_value.process_video_from_streamlit.side_effect = asyncio.TimeoutError
    ...
    with patch("streamlit.session_state", new_callable=MagicMock) as mock_session:
        ...
        assert any(
            "TimeoutError" in e.error_message for e in final_assessment_call_args.errors
        )
