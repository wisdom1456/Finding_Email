...
def discover_documents() -> Tuple[str, List[str]]:
    ...
    if not INTAKE_PATH.exists():
        raise FileNotFoundError(f"Intake form not found: {INTAKE_PATH}")
    
    print(f"📄 INTAKE FORM: {INTAKE_PATH.name}")
        ...
        if "case_analyses" in analysis:
            ...
            for doc_type, docs in doc_types.items():
                ...
                for doc in docs[:2]:  # Show first 2 of each type
                    ...
                    print(f"    • {doc_title} ({summary_length} chars)")
    ...
    try:
        ...
        if analysis_data.get("status") == "success":
            ...
            if "email" in analysis_data:
                ...
                if terms:
                    print(f"🔍 Water damage terms found: {len(terms)} types")
