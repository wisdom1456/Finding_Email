...
DOCUMENTS_PATH = CLIENT_BASE / "Shared Folder with Client" / "Shared with Bernhardt Riley"
...
def discover_documents() -> Tuple[str, List[str]]:
    ...
    if not DOCUMENTS_PATH.exists():
        raise FileNotFoundError(f"Client documents directory not found: {DOCUMENTS_PATH}")
...
def prepare_test_data() -> List[Tuple[str, Tuple[str, bytes, str]]]:
    ...
    print(f"📊 Total payload size: {total_size:,} bytes ({total_size / 1024 / 1024:.1f} MB)")
...
def analyze_response(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if has_analysis:
        ...
        if "case_analyses" in analysis:
            ...
            for i, doc in enumerate(case_docs, 1):
                ...
                flooding_terms = ['flood', 'water', 'damage', 'moisture', 'insurance', 'estimate']
                term_counts = {term: summary.lower().count(term) for term in flooding_terms}
    ...
    if has_email:
        ...
        flooding_terms = ['flood', 'water', 'property', 'damage', 'insurance', 'estimate', 'disclosure']
    ...
    try:
        ...
        if analysis_data.get("status") == "success":
            ...
            if "case_documents" in analysis_data:
                ...
                flooding_docs = sum(1 for doc in analysis_data["case_documents"] 
