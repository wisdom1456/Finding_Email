...
def video_processor(mock_google_cloud):
    ...
    with patch.dict(os.environ, {"GCP_PROJECT_ID": "test-project", "GCP_BUCKET_NAME": "test-bucket"}):
...
class TestVideoProcessor:
    ...
    async def test_process_video_file_success(self, mock_magic, video_processor, temp_video_file):
        ...
        video_processor._analyze_with_vertex_ai = AsyncMock(return_value={"summary": "A cat is playing."})
        video_processor._transcribe_with_speech_to_text = AsyncMock(return_value="Hello world.")
        
        # Mock synchronous methods
        video_processor._upload_to_cloud_storage = MagicMock(return_value="gs://test-bucket/test.mp4")
    ...
    async def test_partial_failure_transcription_fails(self, mock_magic, video_processor, temp_video_file):
        """Test that analysis succeeds even if transcription fails."""
        file_path = temp_video_file()

        video_processor._analyze_with_vertex_ai = AsyncMock(return_value={"summary": "A cat is playing."})
        video_processor._transcribe_with_speech_to_text = AsyncMock(side_effect=VideoProcessingError("Transcription failed"))
        video_processor._upload_to_cloud_storage = MagicMock(return_value="gs://test-bucket/test.mp4")
    ...
    @pytest.mark.asyncio
    async def test_file_validation_error_handling(self, video_processor, temp_video_file):
        ...
        # Test format error
        with patch('backend_logic.video_processor.magic.from_file', return_value='text/plain'):
            invalid_format_file = temp_video_file(filename="test.txt")
            result = await video_processor.process_video_file(invalid_format_file, "test.txt")
            assert isinstance(result, MediaProcessingError)
            assert "Unsupported video format" in result.error_message
    
    @pytest.mark.asyncio
    @patch('backend_logic.video_processor.magic.from_file', return_value='video/mp4')
    async def test_full_analysis_failure(self, mock_magic, video_processor, temp_video_file):
        """Test error handling when the primary Vertex AI analysis fails."""
        file_path = temp_video_file()
        video_processor._upload_to_cloud_storage = MagicMock(return_value="gs://test-bucket/test.mp4")
        video_processor._analyze_with_vertex_ai = AsyncMock(side_effect=VideoProcessingError("Vertex AI failed"))
        # Transcription might still be attempted, we mock it to succeed
        video_processor._transcribe_with_speech_to_text = AsyncMock(return_value="Does not matter")
        video_processor._delete_from_cloud_storage = MagicMock()
    ...
    @pytest.mark.asyncio
    @patch('backend_logic.video_processor.magic.from_file', return_value='video/mp4')
    async def test_concurrent_execution(self, mock_magic, video_processor, temp_video_file):
        """Test that analysis and transcription are called concurrently."""
        with patch('backend_logic.video_processor.asyncio.gather', new_callable=AsyncMock) as mock_gather:
