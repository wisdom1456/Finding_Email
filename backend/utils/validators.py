...
def safe_model_validate(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"VALIDATORS: Fallback validation also failed: {fallback_error}"
            )
            return None
...
def validate_next_steps_formatting(content: str) -> None:
    ...
    if not re.search(r"<strong>", content, re.IGNORECASE):
        raise ValueError(
            "Next steps section is missing deadline formatting (no <strong> tags found)"
        )
...
def validate_section_output(output_content: str, output_format: str) -> None:
    ...
    if output_format == "json":
        try:
            ...
            logger.info(
                f"VALIDATORS: JSON format validation passed for content length {len(output_content)}"
            )
        except json.JSONDecodeError as e:
        ...
        html_pattern = re.search(
            r"<p>|</p>|<ul>|</ul>|<li>|</li>", output_content, re.IGNORECASE
        )
        if not html_pattern:
            logger.error(
                "VALIDATORS: HTML validation failed - no required HTML tags found"
            )
            raise ValueError(
        ...
        logger.info(
            f"VALIDATORS: HTML format validation passed for content length {len(output_content)}"
        )

    else:
        # Handle unknown formats gracefully - log warning but don't fail validation
        logger.warning(
            f"VALIDATORS: Warning - Unknown output format '{output_format}', skipping validation"
        )
