...
class CostCalculator:
    ...
    def calculate_audio_processing_costs(
        ...
        for audio in transcripted_media:
            ...
            if "duration_minutes" in audio_log:
                ...
                actual_minutes = max(
                    1.0, len(audio.transcript) / 200
                )  # ~200 chars per minute speech
    ...
    def calculate_video_processing_costs(
        ...
        for video in video_insights:
            ...
            if "duration_minutes" in video_log:
                ...
                file_size_mb = (
                    video.metadata.size / (1024 * 1024) if video.metadata.size else 50
                )
            ...
            if "gemini_token_usage" in video_log:
                ...
                output_tokens = video_log["gemini_token_usage"].get(
                    "output_tokens", 3000
                )
            else:
            ...
            if (
                isinstance(video, EnhancedVideoInsight)
    ...
    def parse_openai_response_for_tokens(
    ...
    def parse_vertex_ai_response_for_tokens(
        self, response_data: dict[str, Any]
    ) -> dict[str, int]:
        """
        ...
        if "usage_metadata" in response_data:
            ...
            "metadata" in response_data and "tokenMetadata" in response_data["metadata"]
        ):
