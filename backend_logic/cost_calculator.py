...
class CostCalculator:
    ...
    def calculate_document_analysis_costs(
        ...
        for doc in analyzed_documents:
            ...
            document_costs.append(ServiceCost(
    ...
    def calculate_audio_processing_costs(
        ...
        for audio in transcripted_media:
            ...
            audio_costs.append(ServiceCost(
    ...
    def calculate_video_processing_costs(
        ...
        for video in video_insights:
            ...
            video_costs.append(ServiceCost(
            ...
            if 'gemini_token_usage' in video_log:
                ...
                insight_text = str(video.insights) + str(video.labels) + str(video.objects)
                input_tokens = 1500  # Standard prompt
                output_tokens = max(1000, len(insight_text) // 4)
            
            gemini_input_cost = Decimal(str(input_tokens)) * self.PRICING_RATES["vertex_ai_gemini_flash"]["input_tokens"]
            gemini_output_cost = Decimal(str(output_tokens)) * self.PRICING_RATES["vertex_ai_gemini_flash"]["output_tokens"]
            total_gemini_cost = gemini_input_cost + gemini_output_cost
            
            # Calculate weighted rate
            total_tokens = input_tokens + output_tokens
            weighted_rate = total_gemini_cost / Decimal(str(total_tokens)) if total_tokens > 0 else self.PRICING_RATES["vertex_ai_gemini_flash"]["input_tokens"]
            ...
            if isinstance(video, EnhancedVideoInsight) and video.is_criminal_case and video.criminal_analysis:
                ...
                criminal_cost = Decimal(str(criminal_tokens)) * self.PRICING_RATES["vertex_ai_gemini_flash"]["output_tokens"]
                ...
                video_costs.append(ServiceCost(
                    ...
                    unit_type="tokens",
                    rate_per_unit=self.PRICING_RATES["vertex_ai_gemini_flash"]["output_tokens"],
    ...
    def calculate_total_actual_costs(
        self,
        analyzed_documents: List[AnalyzedDocument] = None,
        transcripted_media: List[TranscriptedMedia] = None,
        video_insights: List[VideoInsight] = None,
        processing_logs: Dict[str, Any] = None
    ) -> ActualCosts:
        """
