...
class CostEstimator:
    ...
    def estimate_audio_processing_costs(
        ...
        for audio_file in audio_files:
            ...
            whisper_cost = Decimal(str(estimated_minutes)) * self.PRICING_RATES["openai_whisper"]["per_minute"]
    ...
    def estimate_video_processing_costs(
        ...
        for video_file in video_files:
            ...
            video_cost = Decimal(str(estimated_minutes)) * self.PRICING_RATES["vertex_ai_video"]["per_minute"]
            ...
            gemini_input_cost = Decimal(str(analysis_input_tokens)) * self.PRICING_RATES["vertex_ai_gemini_flash"]["input_tokens"]
            gemini_output_cost = Decimal(str(analysis_output_tokens)) * self.PRICING_RATES["vertex_ai_gemini_flash"]["output_tokens"]
            ...
            video_costs.append(ServiceCost(
                ...
                unit_type="tokens",
                rate_per_unit=self.PRICING_RATES["vertex_ai_gemini_flash"]["input_tokens"],
    ...
    def generate_cost_estimate(
        ...
        if documents:
            estimated_document_costs = self.estimate_document_processing_costs(documents)
