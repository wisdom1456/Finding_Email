...
class DocumentProcessor:
    ...
    async def process_documents_from_streamlit(self, uploaded_files, intake_filenames: List[str]) -> List[ProcessedDocument]:
        """
            ...
            try:
                ...
                if not processor:
                    raise DocumentProcessingError(
                        f"No processor available for file '{uploaded_file.name}' with content type '{content_type}'"
                    )
                
                processing_tasks.append(processor(temp_path, doc_type, uploaded_file.name))
                
            except Exception as e:
                # Clean up temp file on error
                if os.path.exists(temp_path):
                    os.remove(temp_path)
                raise DocumentProcessingError(f"Error processing file '{uploaded_file.name}': {str(e)}")
    ...
    async def process_documents(self, files, intake_filenames: List[str]) -> List[ProcessedDocument]:
        ...
        for file in files:
            ...
            if hasattr(file, 'tmp_path'):
                ...
                raise DocumentProcessingError(f"Unsupported file object type: {type(file)}")
            ...
            if not processor:
                raise DocumentProcessingError(
                    f"No processor available for file '{filename}' with content type '{content_type}'"
                )
