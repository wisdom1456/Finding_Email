...
class ContentFormattingService:
    ...
    def _apply_deadline_formatting(self, content: str) -> str:
        ...
        content = regex_replace_filter(
            content, r"\b(by\s+\w+\s+\d{1,2},?\s+\d{4})\b", r"<strong>\1</strong>"
        )

        # 5. Format deadline references
        content = regex_replace_filter(
            content, r"\b(deadline:?\s*[^.!?]*)\b", r"<strong>\1</strong>"
        )
    ...
    def _clean_ai_response(
        ...
        cleaned = self._strip_citations(
            cleaned
        )  # Secondary citation removal for any missed cases
    ...
    def _apply_enhanced_citation_filtering(self, content: str) -> str:
        ...
        try:
            ...
            if removed_chars > 0:
                ...
                logger.debug(
                    "Citation filtering completed - no citations found to remove"
                )
