...
class CostExporter:
    ...
    def __init__(self, template_dir: str = "backend/assets/templates") -> None:
        """
    ...
    def export_cost_summary_csv(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_variance is not None:
            ...
            writer.writerow(
                ["Cost Variance", f"${float(cost_summary.cost_variance):.4f}"]
            )
            writer.writerow(
                ["Variance Percentage", f"{cost_summary.cost_variance_percentage:.2f}%"]
            )
            writer.writerow(
                [
                    "Variance Status",
                    self._classify_variance(cost_summary.cost_variance_percentage),
                ]
            )
    ...
    def export_cost_summary_json(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_variance is not None:
            export_data["variance_analysis"] = {
                ...
                "variance_classification": self._classify_variance(
                    cost_summary.cost_variance_percentage
                ),
                "variance_explanation": self._generate_variance_explanation(
                    cost_summary
                ),
            }
    ...
    def generate_cost_report_text(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_estimate:
            ...
            lines.append(
                f"Estimation Time: {cost_summary.cost_estimate.estimation_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            lines.append("")
            ...
            if cost_summary.cost_estimate.estimated_document_costs:
                ...
                for cost in cost_summary.cost_estimate.estimated_document_costs:
                    lines.append(
                        f"  • {cost.service_name} ({cost.operation_type}): ${float(cost.total_cost):.4f}"
                    )
        ...
        if cost_summary.actual_costs:
            ...
            lines.append("-" * 15)
            lines.append(
                f"Total Actual Cost: ${float(cost_summary.actual_costs.total_actual_cost):.4f}"
            )
            lines.append(
                f"Processing Time: {cost_summary.actual_costs.processing_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            lines.append("")
            ...
            if cost_summary.actual_costs.document_analysis_costs:
                ...
                for cost in cost_summary.actual_costs.document_analysis_costs:
                    lines.append(
                        f"  • {cost.service_name}: ${float(cost.total_cost):.4f} ({cost.file_name or 'N/A'})"
                    )
        ...
        if cost_summary.cost_variance is not None:
            ...
            lines.append(
                f"Variance Percentage: {cost_summary.cost_variance_percentage:.2f}%"
            )
            lines.append(
                f"Status: {self._classify_variance(cost_summary.cost_variance_percentage)}"
            )
            lines.append(
                f"Analysis: {self._generate_variance_explanation(cost_summary)}"
            )
            lines.append("")
    ...
    def create_budget_analysis(self, cost_summary: CostSummary) -> dict[str, Any]:
            ...
            if estimated > 0:
                ...
                if efficiency_ratio <= 0.95:
                    ...
                    analysis["insights"].append(
                        "Actual costs came in significantly under estimate, indicating excellent cost control."
                    )
                elif efficiency_ratio <= 1.05:
                    analysis["cost_efficiency_score"] = "Good"
                    analysis["insights"].append(
                        "Actual costs were very close to estimates, showing accurate forecasting."
                    )
                elif efficiency_ratio <= 1.15:
                    analysis["cost_efficiency_score"] = "Fair"
                    analysis["insights"].append(
                        "Actual costs exceeded estimates moderately. Review estimation models."
                    )
                else:
                    analysis["cost_efficiency_score"] = "Poor"
                    analysis["insights"].append(
                        "Actual costs significantly exceeded estimates. Immediate review required."
                    )
        ...
        if cost_summary.cost_variance_percentage is not None:
            ...
            if abs(variance_pct) <= 5:
                ...
                analysis["risk_factors"].append(
                    "High cost variance indicates potential budgeting or estimation issues."
                )
        ...
        if cost_summary.actual_costs:
            ...
            if service_breakdown:
                max_service = max(
                    service_breakdown.items(), key=lambda x: x[1]["total_cost"]
                )
                if max_service[1]["percentage"] > 60:
                    analysis["insights"].append(
                        f"{max_service[0]} represents {max_service[1]['percentage']:.1f}% of total costs."
                    )
                    analysis["optimization_opportunities"].append(
                        f"Consider optimizing {max_service[0]} usage for cost reduction."
                    )
        ...
        if cost_summary.actual_costs:
            ...
            if total_costs > 0:
                ...
                analysis["insights"].append(
                    f"Cost distribution: {doc_percentage:.1f}% document analysis, {media_percentage:.1f}% media processing."
                )

                if media_percentage > 40:
                    analysis["optimization_opportunities"].append(
                        "High media processing costs detected. Consider batch processing for efficiency."
                    )
    ...
    def _generate_variance_explanation(self, cost_summary: CostSummary) -> str:
        ...
        variance_pct = cost_summary.cost_variance_percentage
        variance_amount = (
            float(cost_summary.cost_variance) if cost_summary.cost_variance else 0.0
        )
    ...
    def _get_variance_indicators(self, cost_summary: CostSummary) -> dict[str, Any]:
        ...
        indicators = {"color": "gray", "icon": "neutral", "status": "unknown"}
        ...
        if cost_summary.cost_variance_percentage is not None:
            ...
            if abs(variance_pct) <= 5:
                ...
                indicators.update(
                    {"color": "orange", "icon": "alert", "status": "caution"}
                )
            else:
        ...
        if cost_summary.cost_variance_percentage is not None:
            if cost_summary.cost_variance_percentage > 15:
                recommendations.append(
                    "Review and recalibrate cost estimation models for improved accuracy."
                )
                recommendations.append(
                    "Implement stricter budget controls for future cases."
                )
            elif cost_summary.cost_variance_percentage < -10:
                recommendations.append(
                    "Consider if estimation models are too conservative, potentially impacting resource allocation."
                )
        ...
        if cost_summary.actual_costs:
            ...
            for service, data in service_breakdown.items():
                if data["percentage"] > 50:
                    recommendations.append(
                        f"Optimize {service} usage as it represents {data['percentage']:.1f}% of total costs."
                    )
    ...
    def _generate_fallback_html_report(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_variance is not None:
            variance_class = (
                "good"
                if abs(cost_summary.cost_variance_percentage) <= 5
                else "warning"
                if abs(cost_summary.cost_variance_percentage) <= 15
                else "danger"
            )
            html += f"""
            <div class="section">
