...
from legal_portal.core.cost_calculator import CostCalculator
from legal_portal.core.cost_estimator import CostEstimator
from legal_portal.core.cost_exporter import CostExporter
...
class CostSessionManager:
    ...
    def initialize_cost_session(
        ...
        cost_estimate = self.cost_estimator.generate_cost_estimate(
            documents=documents, audio_files=audio_files, video_files=video_files
        )
    ...
    def generate_cost_report(self, case_id: str) -> dict[str, Any]:
        ...
        if cost_summary.cost_variance is not None:
            report["variance_analysis"] = {
                ...
                "variance_status": self._classify_variance(
        ...
        # Service breakdown
        if cost_summary.actual_costs:
            report["service_breakdown"] = self._generate_service_breakdown(
                cost_summary.actual_costs
            )

        return report

    def export_cost_data_for_budget_analysis(
        ...
        for case_id in case_ids:
            ...
            case_data = {
                ...
                if cost_summary.cost_variance_percentage
                else 0.0,
            }
        ...
        export_data["summary_statistics"] = {
            ...
            "average_variance_percentage": sum(variance_list) / len(variance_list)
            if variance_list
            else 0.0,
    ...
    def _save_session(self, case_id: str, cost_summary: CostSummary) -> None:
        ...
        session_data = {
            ...
            "cost_estimate": cost_summary.cost_estimate.model_dump()
            if cost_summary.cost_estimate
            else None,
            "actual_costs": cost_summary.actual_costs.model_dump()
            if cost_summary.actual_costs
            else None,
            "cost_variance": float(cost_summary.cost_variance)
            if cost_summary.cost_variance
    ...
    def _load_session(self, case_id: str) -> CostSummary | None:
    ...
    def export_multiple_sessions(
        self, session_ids: list[str], format: str = "csv"
    ) -> str:
        """
    ...
    def _export_multiple_sessions_csv(self, session_ids: list[str]) -> str:
        ...
        for session_id in session_ids:
            ...
            actual = (
                float(cost_summary.actual_costs.total_actual_cost)
                if cost_summary.actual_costs
                else 0.0
            )
            variance = (
            ...
            status = (
                self._classify_variance(variance_pct)
                if cost_summary.cost_variance_percentage is not None
                else "pending"
            )
