...
try:
    ...
    CV2_AVAILABLE = False
logger.warning(
    "MEDIA PROCESSOR: ⚠️  OpenCV (cv2) not available - video processing will use fallback methods"
)
...
logger.info(
    "MEDIA PROCESSOR: ⚠️  NumPy not available - advanced video analysis will be limited"
)
...
logger.debug(
    "MEDIA PROCESSOR: ⚠️  Google Cloud libraries not available - using local processing only"
)
...
try:
    ...
    PIL_AVAILABLE = False
logger.debug("MEDIA PROCESSOR: ⚠️  PIL not available - image processing will be limited")
...
class MediaProcessor:
    ...
    def process_video_with_google_api(
        self, file_path: str, file_name: str
    ) -> Dict[str, Any]:
        """Process video using Google Cloud Video Intelligence API."""
        logger.debug(f"MEDIA PROCESSOR: 🎥 Processing video: {file_name}")

        if not GOOGLE_CLOUD_AVAILABLE:
            logger.warning(
                "MEDIA PROCESSOR: ⚠️  Google Cloud not available, using fallback analysis"
            )
        ...
        try:
            ...
            if not bucket_name:
                logger.info(
                    "MEDIA PROCESSOR: ⚠️  Google Cloud Storage bucket not configured"
                )
                return self._create_fallback_video_analysis(file_path, file_name)
            ...
            logger.info(
                f"MEDIA PROCESSOR: 🔄 Starting Google Video Intelligence analysis for {file_name}"
            )
            operation = client.annotate_video(request=request)

            # Wait for operation to complete
            logger.info("MEDIA PROCESSOR: ⏳ Waiting for analysis to complete...")
            ...
            logger.error(f"MEDIA PROCESSOR: ❌ Google Video Intelligence error: {e}")
            return self._create_fallback_video_analysis(file_path, file_name)

    def _upload_to_gcs(
        self, file_path: str, file_name: str, bucket_name: str
    ) -> Optional[str]:
        """Upload file to Google Cloud Storage."""
        if not GOOGLE_CLOUD_AVAILABLE:
            logger.info("MEDIA PROCESSOR: ⚠️  Google Cloud Storage not available")
            return None
        ...
        try:
            ...
            logger.info(f"MEDIA PROCESSOR: ☁️  Uploading {file_name} to GCS...")
            blob.upload_from_filename(file_path)

            gcs_uri = f"gs://{bucket_name}/{blob_name}"
            logger.info(f"MEDIA PROCESSOR: ✅ Upload complete: {gcs_uri}")
            return gcs_uri

        except Exception as e:
            logger.error(f"MEDIA PROCESSOR: ❌ GCS upload error: {e}")
            return None
    ...
    def _cleanup_gcs_file(self, gcs_uri: str, bucket_name: str) -> None:
        ...
        if not GOOGLE_CLOUD_AVAILABLE:
            logger.info(
                "MEDIA PROCESSOR: ⚠️  Google Cloud Storage not available for cleanup"
            )
            return
        ...
        try:
            ...
            blob = bucket.blob(blob_name)
            blob.delete()
            logger.info(f"MEDIA PROCESSOR: 🗑️  Cleaned up GCS file: {blob_name}")
        except Exception as e:
            logger.error(f"MEDIA PROCESSOR: ⚠️  GCS cleanup error: {e}")

    def _parse_video_intelligence_results(
        self, result, file_name: str
    ) -> Dict[str, Any]:
        ...
        for annotation in result.annotation_results[0].text_annotations:
            for text_segment in annotation.segments:
                ...
                insights["text_annotations"].append(
                    {"text": annotation.text, "start_time": text}
                )
        ...
        logger.info(
            f"MEDIA PROCESSOR: 📊 Google Video Intelligence analysis complete for {file_name}"
        )
        logger.info(f"MEDIA PROCESSOR: 📊   - Labels: {len(insights['labels'])}")
        logger.info(f"MEDIA PROCESSOR: 📊   - Objects: {len(insights['objects'])}")
        logger.info(
            f"MEDIA PROCESSOR: 📊   - Text annotations: {len(insights['text_annotations'])}"
        )
        logger.info(
            f"MEDIA PROCESSOR: 📊   - Transcript length: {len(insights['transcript'])} chars"
        )
    ...
    def _create_fallback_video_analysis(
        self, file_path: str, file_name: str
    ) -> Dict[str, Any]:
        """Create fallback video analysis using local processing."""
        logger.warning(
            f"MEDIA PROCESSOR: 🔄 Using fallback local analysis for {file_name}"
        )
        ...
        try:
            ...
            if not cap.isOpened():
                logger.info(
                    f"MEDIA PROCESSOR: ❌ Could not open video file: {file_path}"
                )
            ...
            logger.info(f"MEDIA PROCESSOR: 📊 Local analysis complete for {file_name}")
            logger.info(f"MEDIA PROCESSOR: 📊   - Duration: {duration:.1f}s")
            logger.info(f"MEDIA PROCESSOR: 📊   - Frames: {frame_count}")
            ...
            logger.error(f"MEDIA PROCESSOR: ❌ Fallback analysis error: {e}")
            return self._create_empty_video_analysis(file_name)
    ...
    def _sample_video_frames(self, cap, num_samples: int = 5) -> List[np.ndarray]:
        ...
        for i in range(num_samples):
            frame_number = (
                int((i / (num_samples - 1)) * (frame_count - 1))
                if num_samples > 1
                else 0
            )
    ...
    def process_image(self, file_path: str, file_name: str) -> Dict[str, Any]:
        ...
        logger.debug(f"MEDIA PROCESSOR: 🖼️  Processing image: {file_name}")
        ...
        try:
            with Image.open(file_path) as img:
                ...
                dominant_colors = (
                    sorted(colors, key=lambda x: x[0], reverse=True)[:5]
                    if colors
                    else []
                )
                ...
                logger.info(
                    f"MEDIA PROCESSOR: 📊 Image analysis complete for {file_name}"
                )
                logger.info(f"MEDIA PROCESSOR: 📊   - Dimensions: {width}x{height}")
                logger.info(f"MEDIA PROCESSOR: 📊   - Format: {format_name}")
            ...
            logger.error(f"MEDIA PROCESSOR: ❌ Image processing error: {e}")
            return {
                "file_name": file_name,
