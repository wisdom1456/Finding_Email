...
class TemplateRenderingService:
    ...
    def _setup_jinja_environment(self):
        ...
        if self.template_directory and os.path.exists(self.template_directory):
            try:
                ...
                logger.info(
                    f"Jinja2 environment initialized with template directory: {self.template_directory}"
                )
            ...
            logger.warning(
                f"Template directory not found or not provided: {self.template_directory}"
            )
            self.jinja_env = None
    ...
    def prepare_email_context(
        ...
        context = {
            ...
            # Metadata
            "confidence_score": case_data.get("metadata", {}).get(
                "confidence_score", "medium"
            ),
            "generation_timestamp": case_data.get("metadata", {}).get(
