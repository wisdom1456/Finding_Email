...
class TimelineAnalyzer:
    ...
    def extract_timeline_from_content(
        self, content: str, source: str = "unknown"
    ) -> List[Dict[str, Any]]:
        """Extract timeline events from text content."""
        logger.info(f"TIMELINE ANALYZER: 📅 Extracting timeline from {source}")

        if not content or not content.strip():
            logger.info(f"TIMELINE ANALYZER: ⚠️  No content provided for {source}")
            return []
        ...
        logger.info(
            f"TIMELINE ANALYZER: 📅 Extracted {len(events)} timeline events from {source}"
        )
        return events
    ...
    def create_comprehensive_timeline(self, analysis_data: Any) -> Dict[str, Any]:
        ...
        entry_log = {
            ...
            "input_data": {
                ...
                "intake_summary_content": str(
                    getattr(analysis_data, "intake_summary", "N/A")
                )[:500]
                if hasattr(analysis_data, "intake_summary")
                else "N/A",
                "case_documents_count": len(
                    getattr(analysis_data, "case_documents", [])
                )
                if hasattr(analysis_data, "case_documents")
                else 0,
                "video_insights_count": len(
                    getattr(analysis_data, "video_insights", [])
                )
                if hasattr(analysis_data, "video_insights")
                else 0,
            },
        }
        logger.info(json.dumps(entry_log, indent=2))
        logger.debug(
            f"TIMELINE ANALYZER DEBUG - ENTRY LOG: {json.dumps(entry_log, indent=2)}"
        )

        logger.info("TIMELINE ANALYZER: 🗓️  Creating comprehensive timeline")
        ...
        if hasattr(analysis_data, "intake_summary") and analysis_data.intake_summary:
            intake_events = self.extract_timeline_from_content(
                str(analysis_data.intake_summary), "intake_form"
            )
        ...
        if hasattr(analysis_data, "case_documents") and analysis_data.case_documents:
            for i, doc in enumerate(analysis_data.case_documents):
                ...
                doc_events = self.extract_timeline_from_content(
                    doc_content, f"document_{doc_name}"
                )
            ...
            for video in analysis_data.video_insights:
                if video.transcript:
                    video_events = self.extract_timeline_from_content(
                        video.transcript, f"video_{video.file_name}"
                    )
        ...
        logger.info(
            f"TIMELINE ANALYZER: 🗓️  Timeline created with {len(unique_events)} events"
        )
        logger.info(f"TIMELINE ANALYZER: 🗓️  Sources: {', '.join(sources_processed)}")
        ...
        exit_log = {
            ...
            "response_data": {
                "total_events": len(unique_events),
                "sources_processed": sources_processed,
                "events_sample": unique_events[:3]
                if unique_events
                else [],  # First 3 events for sample
        ...
        logger.debug(
            f"TIMELINE ANALYZER DEBUG - EXIT LOG: {json.dumps(exit_log, indent=2)}"
        )
    ...
    def _parse_date_string(self, date_string: str) -> Optional[datetime]:
        ...
        try:
            ...
            logger.error(
                f"TIMELINE ANALYZER: ⚠️  Date parsing failed for '{date_string}': {e}"
            )
    ...
    def _get_sentence_context(
        self, sentences: List[str], index: int, window: int = 1
    ) -> str:
    ...
    def _deduplicate_events(self, events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for event in events:
            ...
            date_str = (
                event["date"].strftime("%Y-%m-%d") if event["date"] else "no_date"
            )
            ...
            if key not in seen_combinations:
                ...
                for existing_event in unique_events:
                    existing_date_str = (
                        existing_event["date"].strftime("%Y-%m-%d")
                        if existing_event["date"]
                        else "no_date"
                    )
                    existing_desc_key = (
                        existing_event["description"][:50].lower().strip()
                    )
                    ...
                    if existing_key == key:
                        ...
                        existing_event["confidence"] = max(
                            existing_event["confidence"], event["confidence"]
                        )
                        break
    ...
    def _find_date_clusters(
        self, dated_events: List[Dict[str, Any]], max_gap_days: int = 7
    ) -> List[Dict[str, Any]]:
            ...
            if not current_cluster:
                ...
                if gap_days <= max_gap_days:
                    ...
                    if len(current_cluster) > 1:
                        clusters.append(
                            {
                                "start_date": current_cluster[0]["date"].strftime(
                                    "%Y-%m-%d"
                                ),
                                "end_date": current_cluster[-1]["date"].strftime(
                                    "%Y-%m-%d"
                                ),
    ...
    def format_timeline_for_prompt(
        self, timeline_data: Dict[str, Any], max_events: int = 20
    ) -> str:
        ...
        for event in events:
            date_str = (
                event["date"].strftime("%Y-%m-%d") if event["date"] else "Date unknown"
            )
            source = event["source"]
            description = (
            ...
            formatted_lines.append(
                f"- {date_str} ({source}, confidence: {confidence:.1f}): {description}"
            )
        ...
        if timeline_data.get("analysis", {}).get("gaps"):
            ...
            for gap in timeline_data["analysis"]["gaps"][:3]:  # Top 3 gaps
                formatted_lines.append(
                    f"- {gap['gap_days']} day gap between {gap['start_date']} and {gap['end_date']}"
                )
