...
class VideoProcessor:
    ...
    def _upload_to_cloud_storage(self, file_path: str, file_name: str) -> str:
        try:
            bucket = self.storage_client.bucket(self.bucket_name)
            unique_name = (
                f"{self.temp_folder}/{uuid.uuid4()}-{os.path.basename(file_name)}"
            )
...
CONSTITUTIONAL FOCUS: Emphasize 4th Amendment (search/seizure), 5th Amendment (self-incrimination), 6th Amendment (counsel) issues"""
    ...
    async def _transcribe_with_speech_to_text(
    ...
    def _parse_criminal_analysis(
        self, analysis_result: dict[str, Any]
    ) -> CriminalVideoAnalysis | None:
            ...
            for item_data in raw_evidence_items:
                try:
                    ...
                    evidence_item = CriminalEvidenceItem(
                        ...
                        constitutional_issues=item_data.get(
                            "constitutional_issues", []
                        ),
                        evidence_strength=item_data.get(
    ...
    async def process_video_file(
        ...
        try:
            ...
            analysis_task = self._analyze_with_vertex_ai(
                gcs_uri, file_name, is_criminal_case
            )
            transcription_task = self._transcribe_with_speech_to_text(
                gcs_uri, file_name
            )

            analysis_result, transcription_result = await asyncio.gather(
                analysis_task, transcription_task
            )
