...
def main():
    ...
    logger.debug("🔍 Debugging Actual JSON Structure Generation")
    logger.info("=" * 50)
    ...
    try:
        ...
        if json_data:
            ...
            logger.debug("📁 Raw JSON saved to: debug_raw_json_structure.json")
            ...
            if "generated_letter" in json_data:
                ...
                logger.info(
                    f"  ✅ generated_letter present with keys: {list(gl.keys())}"
                )
                logger.info(
                    f"  📝 background_summary present: {'background_summary' in gl}"
                )
                logger.info(
                    f"  📝 analysis_and_position present: {'analysis_and_position' in gl}"
                )
                if "background_summary" in gl:
                    logger.info(
                        f"  📏 background_summary length: {len(str(gl['background_summary']))}"
                    )
                if "analysis_and_position" in gl:
                    logger.info(
                        f"  📏 analysis_and_position length: {len(str(gl['analysis_and_position']))}"
                    )
            else:
                logger.info("  ❌ generated_letter missing!")

                # Check bridges
            if "bridges" in json_data:
                bridges = json_data["bridges"]
                logger.info(f"  ✅ bridges present with keys: {list(bridges.keys())}")
            else:
                logger.info("  ❌ bridges missing!")

                # Check claims
            if "claims" in json_data:
                claims = json_data["claims"]
                logger.info(
                    f"  ✅ claims present, count: {(len(claims) if isinstance(claims, list) else 'not a list')}"
                )
            else:
                logger.info("  ❌ claims missing!")

                # Check next_steps
            if "next_steps" in json_data:
                ns = json_data["next_steps"]
                logger.debug(
                    f"  ✅ next_steps present with keys: {(list(ns.keys()) if isinstance(ns, dict) else 'not a dict')}"
                )
            else:
                logger.debug("  ❌ next_steps missing!")
