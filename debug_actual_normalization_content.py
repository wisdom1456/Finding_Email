...
def analyze_long_sentences(sentences):
    ...
    logger.info(f"Found {len(long_sentences)} sentences with >15 words:")
    for i, (sent_num, word_count, sentence) in enumerate(
        long_sentences[:10]
    ):  # Show first 10
...
def analyze_repeated_phrases(phrase_counts):
    ...
    logger.info(f"Found {len(repeated)} phrases repeated >2 times:")
    for i, (phrase, count) in enumerate(repeated[:15]):  # Show top 15
        logger.info(f"{i + 1}. '{phrase}' appears {count} times")
...
def main():
    """Debug script to examine the actual content that's failing normalization."""
    # Create a test case
    logger.info("🧪 Analyzing actual generated content for normalization issues...")
    ...
    intake = EnhancedIntakeAnalysis(
        ...
        key_facts=[
            "Police stop without probable cause",
    ...
    try:
        ...
        duplicates, long_count, repeated_count, sentences, phrase_counts = (
            count_normalization_issues(html_content)
        )

        logger.info("📊 SUMMARY:")
        logger.info(f"  - Duplicate sentences: {duplicates}")
        logger.info(f"  - Long sentences (>15 words): {long_count}")
        logger.info(f"  - Repeated 3-word phrases (>2 times): {repeated_count}")
        logger.info(f"  - Total sentences: {len(sentences)}")
        logger.info(f"  - Total unique 3-word phrases: {len(phrase_counts)}")
