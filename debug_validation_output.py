...
def main():
    logger.debug("🔍 Debug Validation Output - Checking Template Rendering")
    logger.info("=" * 60)
    ...
    logger.info("🧪 Testing case: Complex Civil Rights Case")
    ...
    if not email_result:
        logger.info("❌ Email generation returned None")
        return

    # EmailGeneratorV2 returns dict with 'main_letter' and 'appendix' keys
    generated_email = (
        email_result["main_letter"] if isinstance(email_result, dict) else email_result
    )
    analysis_doc = (
        email_result.get("appendix", "") if isinstance(email_result, dict) else ""
    )

    logger.info(f"\n📄 Generated Email Length: {len(generated_email)} characters")
    logger.info(f"📄 Analysis Doc Length: {len(analysis_doc)} characters")
    ...
    logger.info("\n🔍 Section Analysis:")
    logger.info(f"   H2 sections found: {len(h2_sections)}")
    logger.info(f"   H3 sections found: {len(h3_sections)}")
    logger.info(f"   Total sections: {len(h2_sections) + len(h3_sections)}")

    if h2_sections:
        logger.info("\n📋 H2 Section Titles:")
        for i, h2 in enumerate(h2_sections, 1):
            logger.info(f"   {i}. '{h2.get_text().strip()}'")

    if h3_sections:
        logger.info("\n📋 H3 Section Titles:")
        for i, h3 in enumerate(h3_sections, 1):
            logger.info(f"   {i}. '{h3.get_text().strip()}'")
    ...
    if section_divs:
        ...
        for i, div in enumerate(section_divs, 1):
            logger.info(f"   {i}. '{div.get_text().strip()}'")
    ...
    logger.info("\n💾 Saved outputs:")
    logger.debug("   Email: debug_validation_email_output.html")
    logger.debug("   Analysis: debug_validation_analysis_output.html")

    # Show first 500 chars for quick inspection
    logger.info("\n📝 Email Preview (first 500 chars):")
    logger.info(generated_email[:500])
    logger.info("...")
