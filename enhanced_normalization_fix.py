...
def enhanced_normalization_fixes(html_content):
    ...
    try:
        logger.debug("🔧 Starting enhanced normalization processing...")
        ...
        for element in soup.find_all(["p", "li", "div"]):
            for text_node in element.find_all(text=True):
                ...
                ) and text_node.parent.name not in ["script", "style"]:
        ...
        for element in soup.find_all(["p", "li", "div"]):
            for text_node in element.find_all(text=True):
                if isinstance(
                    ...
                    new_text = apply_phrase_replacements(
                        original_text, phrase_replacements
                    )
                    if new_text != original_text:
                        text_node.replace_with(new_text)

        logger.debug("✅ Enhanced normalization processing completed")
        return str(soup)

    except Exception as e:
        logger.error(f"❌ Enhanced normalization processing failed: {e}")
        return html_content
...
def break_sentence_at_natural_points(sentence):
    ...
    break_patterns = [
        (
            r",\s+(and|but|however|moreover|furthermore|additionally|therefore|thus|consequently)\s+",
            2,
        ),
        (r";\s+", 1),
    ...
    for pattern, min_words_before in break_patterns:
        ...
        if match:
            ...
            if (
                len(before.split()) >= min_words_before
                and len(after.split()) >= min_words_before
            ):
...
if __name__ == "__main__":
    ...
    logger.info(
        f"BEFORE: Duplicates: {dup_before}, Long sentences: {long_before}, Repeated phrases: {repeated_before}"
    )
    ...
    logger.info(
        f"AFTER: Duplicates: {dup_after}, Long sentences: {long_after}, Repeated phrases: {repeated_after}"
    )

    logger.info("\n📊 IMPROVEMENT:")
    logger.info(
        f"Duplicates: {dup_before} → {dup_after} ({dup_before - dup_after} improvement)"
    )
    logger.info(
        f"Long sentences: {long_before} → {long_after} ({long_before - long_after} improvement)"
    )
    logger.info(
        f"Repeated phrases: {repeated_before} → {repeated_after} ({repeated_before - repeated_after} improvement)"
    )

    logger.info("\n📝 ENHANCED OUTPUT:")
    logger.info("=" * 50)
    logger.info(enhanced_html)
