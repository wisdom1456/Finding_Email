            ...
            for word in words:
                ...
                if len(current_chunk) >= max_words and word.endswith(","):
                    ...
                    for j in range(max_words - 1, max(0, max_words - 5), -1):
                        if j < len(current_chunk) and current_chunk[j] in [
                            "and",
                            "or",
                            "but",
                            "with",
...
def normalize_html_content(html_content):
    ...
    logger.info("🔧 Applying normalization fixes...")
...
def main():
    ...
    try:
        ...
        logger.debug("📄 Processing normalization content...")
        ...
        logger.debug(
            "✅ Normalized content saved to: debug_normalization_content_fixed.html"
        )
        ...
        logger.info(
            f"📊 Long sentences (>15 words): {original_long} → {normalized_long}"
        )

        if normalized_long < original_long:
            logger.info("✅ Improvement achieved!")
        else:
            logger.debug("⚠️  May need additional processing")

    except FileNotFoundError:
        logger.debug("❌ debug_normalization_content.html not found")
        logger.debug("   Run debug_normalization_issues.py first")
