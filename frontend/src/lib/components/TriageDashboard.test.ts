...
describe('TriageDashboard', () => {
    ...
    it('shows all-clear message when all docs are ready and signed', () => {
        render(TriageDashboard, { props: { documents: [signedDoc], activeFilters: new Set(), onFilterToggle: vi.fn() } });
        expect(screen.getByText(/verified and ready/i)).toBeTruthy();
    });

    it('shows attention count when problems exist', () => {
        render(TriageDashboard, { props: { documents: [signedDoc, unsignedDoc], activeFilters: new Set(), onFilterToggle: vi.fn() } });
