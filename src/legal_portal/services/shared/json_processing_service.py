...
class JsonProcessingService:
    ...
    def _findings_writer_instructions(self) -> str:
        ...
        return (
            ...
            "Write in a conversational professional tone — clear, reassuring, and direct. "
            "Structure the email with four clearly labeled sections: "
            "BACKGROUND & ISSUE, KEY LEGAL ISSUES, ANALYSIS, and RECOMMENDED NEXT STEPS. "
            "In BACKGROUND & ISSUE, restate the client's account and findings from document review. "
            "In KEY LEGAL ISSUES, present each applicable legal doctrine as a bold-titled analytical "
            "paragraph combining: the rule, why it matters here, and what it means for the client. "
            "Do not present issues as a bare list of doctrines — each must be a short paragraph. "
            "Cite controlling authority where available. "
            "Keep KEY LEGAL ISSUES limited to substantive legal theories — address procedural matters "
            "in ANALYSIS or RECOMMENDED NEXT STEPS. "
            "In ANALYSIS, apply the issues to the client's specific facts, assess evidence strength, "
            "and note the primary evidentiary gap or risk. "
            "In RECOMMENDED NEXT STEPS, state the recommended course of action and include "
            "a scope-of-engagement paragraph. "
            "Every factual statement must come from the provided sources — never invent details. "
            "Reference documents naturally in prose without parenthetical citation labels. "
            "Target 700-1,200 words. Close with a call to action and confidentiality statement."
        )
    ...
    def _create_structure_instruction(self, structure_guidance) -> str:
        ...
        instructions = (
            ...
            "  1. BACKGROUND & ISSUE (restate client's account and document findings)\n"
            "  2. KEY LEGAL ISSUES (bold-titled analytical paragraph for each doctrine: rule + relevance + client impact)\n"
            "  3. ANALYSIS (apply issues to facts, assess evidence, note primary risk)\n"
            ...
            "- In KEY LEGAL ISSUES, each issue must be a short analytical paragraph, not a bare list.\n"
    ...
    def _build_balanced_client_strategy_directives(self, strategy_object: Optional[Dict[str, Any]]) -> str:
        ...
        return (
            ...
            "- Write for a client. Keep tone conversational professional — clear, reassuring, and direct.\n"
            "- Use the four-section structure: BACKGROUND & ISSUE, KEY LEGAL ISSUES, ANALYSIS, RECOMMENDED NEXT STEPS.\n"
            "- In KEY LEGAL ISSUES, present theories from strategy_object.ranked_theories as bold-titled "
            "analytical paragraphs combining: rule, relevance, and client impact.\n"
            "- Do not present issues as a bare list — each must be a short paragraph.\n"
            "- Give each applicable doctrine substantive treatment — do not compress secondary theories.\n"
            "- In ANALYSIS, apply the issues to the client's facts. Discuss the primary theory "
