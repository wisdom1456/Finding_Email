...
class AIAnalyzer:
    ...
    async def analyze_intake(self, intake_doc: ProcessedDocument) -> CaseAnalysisResult:
        ...
        logger.info("AI ANALYZER: 🔍 Starting intake analysis with modular architecture")
        ...
        try:
            logger.info(f"AI ANALYZER: 🔍 Building intake prompt for: {intake_doc.file_name}")
            ...
            logger.error(f"AI ANALYZER: ❌ Unexpected error during intake analysis: {e}")
            analysis.errors.append(
                AnalysisError(
                    source="IntakeAnalysis",
    ...
    async def analyze_case_documents(
        ...
        for i, doc in enumerate(documents, 1):
            logger.debug(f"AI ANALYZER: Processing document {i}/{total_docs}: {doc.file_name}")
    ...
    async def perform_final_assessment(self, analysis: CaseAnalysisResult) -> CaseAnalysisResult:
        ...
        try:
            logger.info("AI ANALYZER: Starting final legal assessment with modular architecture...")
            ...
            if estimated_tokens > 120000:
                ...
                analysis_for_assessment = self.token_manager.truncate_video_content_aggressively(
            ...
            if "legal_assessment" in raw_assessment:
                ...
                if validated_assessment:
                    ...
                    logger.error("AI ANALYZER: ⚠️  Legal assessment validation failed, using fallback")
            ...
            if "demand_letter_evaluation" in raw_assessment:
                ...
                if validated_evaluation:
                    ...
                    logger.error("AI ANALYZER: ⚠️  Demand letter evaluation validation failed, using fallback")
                    analysis.demand_letter_evaluation = DemandLetterEvaluation.model_validate(
                        create_fallback_demand_letter_evaluation()
                    )
            else:
                logger.warning("AI ANALYZER: ⚠️  No demand_letter_evaluation in response, using fallback")
                analysis.demand_letter_evaluation = DemandLetterEvaluation.model_validate(
                    create_fallback_demand_letter_evaluation()
                )
