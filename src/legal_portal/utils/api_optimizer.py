...
class OpenAIOptimizer:
    ...
    def _cached_completion(
        self, cache_key: str, prompt: str, model: str, temperature: float = 0
    ) -> APICallResult:
        ...
        with self.executor as executor:
            for i, prompt in enumerate(prompts):
                ...
                if use_cache and temperature == 0:
                    ...
                    future = executor.submit(self._cached_completion, cache_key, prompt, model, temperature)
                else:
                    # Non-cacheable completion
                    future = executor.submit(self._single_completion, prompt, model, temperature)
                futures[future] = i
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    results[idx] = APICallResult(
                        content=f"Error: {e!s}", model=model, tokens_used=0, latency=0, cached=False
                    )
    ...
    async def async_batch_completions(
        ...
        return await loop.run_in_executor(
            None, self.batch_completions, prompts, model, use_cache, temperature, progress_callback
        )
