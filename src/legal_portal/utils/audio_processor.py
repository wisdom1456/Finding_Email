...
class AudioProcessor:
    ...
    def _validate_audio_file(self, file_path: str, file_name: str) -> None:
        ...
        if not os.path.exists(file_path):
            msg = f"Audio file not found: {file_name}"
        ...
        try:
            ...
            logger.info(f"AUDIO PROCESSOR: Warning - Could not detect MIME type for {file_name}: {e}")
    ...
    async def process_audio_file(
    ...
    ) -> TranscriptedMedia | MediaProcessingError:
        ...
        try:
            ...
            metadata = FileMetadata(filename=file_name, content_type=mime_type, size=file_size)
    ...
    async def process_audio_from_streamlit(
        self, uploaded_file, file_name: str
    ) -> TranscriptedMedia | MediaProcessingError:
        ...
        try:
            ...
            if temp_path and os.path.exists(temp_path):
                try:
                    ...
                    logger.info(f"AUDIO PROCESSOR: Warning - Could not delete temp file {temp_path}: {e}")


# Convenience function for direct integration with existing file processing pipeline
async def process_audio(
    file_path: str, document_type, file_name: str, openai_client: OpenAI = None
) -> TranscriptedMedia:
    ...
    if openai_client is None:
        msg = "OpenAI client is required for audio processing"
        raise AudioProcessingError(msg)
    ...
    if isinstance(result, MediaProcessingError):
        msg = f"Audio processing failed: {result.error_message}"
        raise AudioProcessingError(msg)
