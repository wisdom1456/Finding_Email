...
class CostExporter:
    ...
    def export_cost_summary_json(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_estimate:
            export_data["cost_estimate"] = {
                "total_estimated_cost": float(cost_summary.cost_estimate.estimated_cost),
    ...
    def generate_cost_report_text(self, cost_summary: CostSummary) -> str:
        ...
        if cost_summary.cost_estimate:
            ...
            lines.append("-" * 20)
            lines.append(f"Total Estimated Cost: ${float(cost_summary.cost_estimate.estimated_cost):.4f}")
    ...
    def _generate_fallback_html_report(self, cost_summary: CostSummary) -> str:
        ...
        # Define CSS styles separately to avoid f-string conflicts
        css_styles = (
        ...
        # Use string formatting instead of f-string to avoid CSS brace conflicts
        html = """<!DOCTYPE html>
<html>
<head>
    <title>Budget Report - {case_id}</title>
    <style>
{css_styles}
    </style>
</head>
<body>
        ...
        if cost_summary.cost_estimate:
            ...
            <div class="section">
                <h2>Cost Estimate</h2>
                <p><strong>Total Estimated:</strong> ${estimated_cost:.4f}</p>
            </div>
            """.format(estimated_cost=float(cost_summary.cost_estimate.estimated_cost))

        if cost_summary.actual_costs:
            html += """
            <div class="section">
                <h2>Actual Costs</h2>
                <p><strong>Total Actual:</strong> ${actual_cost:.4f}</p>
            </div>
            """.format(actual_cost=float(cost_summary.actual_costs.total_actual_cost))
        ...
        if cost_summary.cost_variance is not None:
            ...
            <div class="section">
                ...
                <div class="variance {variance_class}">
                    <p><strong>Variance:</strong> ${variance:.4f} ({variance_pct:.2f}%)</p>
                    <p><strong>Status:</strong> {status}</p>
                </div>
            </div>
            """.format(
                variance_class=variance_class,
                variance=float(cost_summary.cost_variance),
                variance_pct=cost_summary.cost_variance_percentage,
                status=self._classify_variance(cost_summary.cost_variance_percentage),
            )
