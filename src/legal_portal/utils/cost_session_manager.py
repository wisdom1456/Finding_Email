...
class CostSessionManager:
    ...
    def export_cost_data_for_budget_analysis(self, case_ids: list[str] | None = None) -> dict[str, Any]:
        ...
        for case_id in case_ids:
            ...
            if cost_summary.actual_costs and cost_summary.actual_costs.total_actual_cost is not None:
    ...
    def export_multiple_sessions(self, session_ids: list[str], format: str = "csv") -> str:
    ...
    def _export_multiple_sessions_csv(self, session_ids: list[str]) -> str:
        ...
        for session_id in session_ids:
            ...
            estimated = (
                float(cost_summary.cost_estimate.estimated_cost) if cost_summary.cost_estimate else 0.0
            )
            ...
            if cost_summary.actual_costs:
                doc_costs = float(
                    sum(cost.total_cost for cost in cost_summary.actual_costs.document_analysis_costs)
                )
                media_costs = float(
                    sum(cost.total_cost for cost in cost_summary.actual_costs.media_processing_costs)
                )
