...
from legal_portal.config.configuration_manager import ConfigurationManager
from legal_portal.core.data_models import CaseAnalysisResult
from legal_portal.services.content_generation_service import ContentGenerationService
from legal_portal.services.fallback_generation_service import FallbackGenerationService
...
from legal_portal.services.text_processing_service import TextProcessingService
...
class EmailReadabilityError(Exception):
    """Exception raised when email generation fails to produce readable content."""
...
class EmailGeneratorV2:
    ...
    def __init__(
        ...
        if openai_api_key:
            try:
                ...
                self.api_optimizer = OpenAIOptimizer(
                    api_key=openai_api_key, max_workers=max_concurrent_requests
                )
        ...
        if self.openai_client and self.config:
            ...
            logger.warning("JsonProcessingService not initialized - missing OpenAI client or config")
            self.json_processing_service = None

        # Initialize content generation service with refactored dependencies
        self.content_service = ContentGenerationService(json_processing_service=self.json_processing_service)
    ...
    def generate_email_and_analysis_docs(self, case_analysis: CaseAnalysisResult) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.json_processing_service:
                ...
                return self._generate_fallback_response(case_analysis, "Service initialization failed")

            # Validate configuration
            if not self.config_manager.is_configured():
                logger.warning("Configuration not fully loaded, using fallback")
                return self._generate_fallback_response(case_analysis, "Configuration incomplete")
            ...
            result_debug = {
                ...
                "has_letter_content": "letter_content" in result if isinstance(result, dict) else False,
            ...
            if result.get("letter_content") and self.template_directory:
                try:
                    ...
                    rendered_content = self._apply_optional_template_formatting(result["letter_content"])
                    if rendered_content:
    ...
    def _apply_optional_template_formatting(self, html_content: str) -> Optional[str]:
        ...
        try:
            ...
            template_debug_log = {
                ...
                "template_exists": self.template_service.template_exists("findings_email.jinja2"),
            ...
            logger.info("Bypassing Jinja2 template rendering - JsonProcessingService returns complete HTML")
    ...
    def _generate_fallback_response(
        ...
        letter_content = self.fallback_service.create_fallback_letter(case_data, error_message)
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fallback_service": {"available_strategies": self.fallback_service.error_recovery_strategies},
        }

    def process_documents_batch(
        self, documents: List[CaseAnalysisResult], progress_callback: Optional[Callable] = None
    ) -> List[Dict[str, Any]]:
        ...
        if uncached_indices:
            with ThreadPoolExecutor(max_workers=self.max_concurrent_requests) as executor:
                ...
                for idx in uncached_indices:
                    future = executor.submit(self._process_single_document_optimized, documents[idx])
                    futures[future] = idx
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        results[idx] = self._generate_fallback_response(documents[idx], str(e))
