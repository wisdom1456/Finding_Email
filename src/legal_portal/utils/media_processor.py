...
try:
    ...
    CV2_AVAILABLE = False
logger.warning("MEDIA PROCESSOR: ⚠️  OpenCV (cv2) not available - video processing will use fallback methods")
...
logger.info("MEDIA PROCESSOR: ⚠️  NumPy not available - advanced video analysis will be limited")
...
logger.debug("MEDIA PROCESSOR: ⚠️  Google Cloud libraries not available - using local processing only")
...
class MediaProcessor:
    ...
    def process_video_with_google_api(self, file_path: str, file_name: str) -> Dict[str, Any]:
        ...
        if not GOOGLE_CLOUD_AVAILABLE:
            logger.warning("MEDIA PROCESSOR: ⚠️  Google Cloud not available, using fallback analysis")
        ...
        try:
            ...
            if not bucket_name:
                logger.info("MEDIA PROCESSOR: ⚠️  Google Cloud Storage bucket not configured")
                return self._create_fallback_video_analysis(file_path, file_name)
            ...
            logger.info(f"MEDIA PROCESSOR: 🔄 Starting Google Video Intelligence analysis for {file_name}")
    ...
    def _cleanup_gcs_file(self, gcs_uri: str, bucket_name: str) -> None:
        ...
        if not GOOGLE_CLOUD_AVAILABLE:
            logger.info("MEDIA PROCESSOR: ⚠️  Google Cloud Storage not available for cleanup")
            return
    ...
    def _parse_video_intelligence_results(self, result, file_name: str) -> Dict[str, Any]:
        ...
        for annotation in result.annotation_results[0].text_annotations:
            for text_segment in annotation.segments:
                ...
                insights["text_annotations"].append({"text": annotation.text, "start_time": text})
        ...
        logger.info(f"MEDIA PROCESSOR: 📊 Google Video Intelligence analysis complete for {file_name}")
        logger.info(f"MEDIA PROCESSOR: 📊   - Labels: {len(insights['labels'])}")
        logger.info(f"MEDIA PROCESSOR: 📊   - Objects: {len(insights['objects'])}")
        logger.info(f"MEDIA PROCESSOR: 📊   - Text annotations: {len(insights['text_annotations'])}")
        logger.info(f"MEDIA PROCESSOR: 📊   - Transcript length: {len(insights['transcript'])} chars")
    ...
    def _create_fallback_video_analysis(self, file_path: str, file_name: str) -> Dict[str, Any]:
        ...
        logger.warning(f"MEDIA PROCESSOR: 🔄 Using fallback local analysis for {file_name}")
        ...
        try:
            ...
            if not cap.isOpened():
                logger.info(f"MEDIA PROCESSOR: ❌ Could not open video file: {file_path}")
    ...
    def process_image(self, file_path: str, file_name: str) -> Dict[str, Any]:
        ...
        try:
            with Image.open(file_path) as img:
                ...
                logger.info(f"MEDIA PROCESSOR: 📊 Image analysis complete for {file_name}")
