...
class PIISanitizer:
    ...
    PII_PATTERNS: List[Tuple[Pattern, str, str]] = [
        ...
        (
            re.compile(
                r"\b(?:SSN|Social Security Number|Social Security #|SS#)[:\s]*(\d{3}[-.\s]?\d{2}[-.\s]?\d{4})\b",
                re.IGNORECASE,
            ),
            "[SSN_REDACTED]",
        ...
        (
            re.compile(
                r"\b(?:EIN|Employer Identification Number|Tax ID)[:\s]*(\d{2}[-.\s]?\d{7})\b", re.IGNORECASE
            ),
            "[EIN_REDACTED]",
        ...
        (
            re.compile(
                r"\b(?:DL|Driver\'s License|Driver License|License #)[:\s]*([A-Z]\d{3}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}|\d{7,12}|[A-Z]{1,2}\d{5,8})\b",
                re.IGNORECASE,
            ),
            "[DL_NUMBER_REDACTED]",
        ...
        (
            re.compile(
                r"\b(?:Passport|Passport Number|Passport #)[:\s]*([A-Z]{1,2}\d{6,9})\b", re.IGNORECASE
            ),
            "[PASSPORT_REDACTED]",
            "Passport Numbers",
        ),
        ...
        (
            re.compile(
                r"\b(?:Mr\.|Mrs\.|Ms\.|Dr\.|Prof\.)\s+[A-Z][a-z]+(?:\s+[A-Z]\.?\s*)?(?:\s+[A-Z][a-z]+)+\b"
            ),
            "[CLIENT_NAME]",
            "Names with titles",
        ),
        ...
        (
            re.compile(
                r"(?:Plaintiff|Defendant|Petitioner|Respondent|Appellant|Appellee)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
                re.IGNORECASE,
            ),
        ...
        (
            re.compile(
                r"\b(?:Account|Acct|Acc)\.?\s*(?:No\.?|Number|#)?\s*[:\s]*(\d{8,17})\b", re.IGNORECASE
            ),
            "[ACCOUNT_NUMBER]",
        ...
        (
            re.compile(
                r"\$\s*[\d,]+(?:\.\d{2})?(?:\s*(?:USD|dollars?|million|thousand|hundred))?", re.IGNORECASE
            ),
            "[MONETARY_AMOUNT]",
            "Monetary amounts",
        ),
        ...
        (
            re.compile(
                r"(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}(?:\s*(?:ext|x|extension)\.?\s*\d{1,5})?",
                re.IGNORECASE,
            ),
            "[PHONE]",
        ...
        (
            re.compile(
                r"\b\d{1,5}\s+(?:[NSEW]\.?\s+)?[A-Za-z\s]+(?:Street|St\.?|Avenue|Ave\.?|Road|Rd\.?|Boulevard|Blvd\.?|Lane|Ln\.?|Drive|Dr\.?|Court|Ct\.?|Place|Pl\.?|Circle|Cir\.?|Trail|Trl\.?|Way|Parkway|Pkwy\.?|Highway|Hwy\.?|Plaza|Square|Sq\.?)(?:\s+(?:Suite|Ste\.?|Apt\.?|Unit|#)\s*\d+[A-Za-z]?)?\b",
                re.IGNORECASE,
            ),
            "[STREET_ADDRESS]",
        ...
        (re.compile(r"\b\d{5}(?:[-\s]\d{4})?\b"), "[ZIP_CODE]", "ZIP codes"),
        # Legal-Specific Information
        (
            re.compile(
                r"(?:Case|Docket|File|Matter|Cause)\.?\s*(?:No\.?|Number|#)?\s*[:\s]*([A-Z0-9]{2,}[-\s]?[A-Z0-9]+(?:[-\s][A-Z0-9]+)*)",
                re.IGNORECASE,
            ),
            "[CASE_NUMBER]",
            "Legal case numbers",
        ...
        (
            re.compile(
                r"(?:Index|Indictment|Citation)\.?\s*(?:No\.?|Number|#)?\s*[:\s]*(\d+[-/]?\d+)", re.IGNORECASE
            ),
            "[LEGAL_INDEX_NUMBER]",
        ...
        (
            re.compile(
                r"\b(?:United States|U\.S\.|US|State of [A-Z][a-z]+|County of [A-Z][a-z]+)\s+(?:District|Circuit|Superior|Municipal|County|State|Federal|Bankruptcy|Tax|Family|Probate|Juvenile|Criminal|Civil)\s+Court\b",
                re.IGNORECASE,
            ),
            "[COURT_NAME]",
        ...
        (
            re.compile(
                r"\b(?:Judge|Justice|Magistrate|Hon\.|Honorable)\s+[A-Z][a-z]+(?:\s+[A-Z]\.?\s*)?(?:\s+[A-Z][a-z]+)+\b",
                re.IGNORECASE,
            ),
            "[JUDGE_NAME]",
            "Judge names",
        ...
        (
            re.compile(
                r"(?:Attorney|Counsel|Lawyer|Esq\.|Esquire)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
                re.IGNORECASE,
            ),
            "[ATTORNEY_NAME]",
            "Attorney names",
        ...
        (
            re.compile(
                r"(?:Law Firm|Law Office|Legal Services|Attorneys at Law)[:\s]*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+(?:LLP|LLC|PC|PA|PLLC|P\.C\.|P\.A\.))?)",
                re.IGNORECASE,
            ),
            "[LAW_FIRM]",
            "Law firm names",
        ...
        (
            re.compile(
                r"(?:Bar|License|Attorney)\.?\s*(?:No\.?|Number|#)?\s*[:\s]*(\d{5,10})", re.IGNORECASE
            ),
            "[BAR_NUMBER]",
        ...
        (
            re.compile(
                r"(?:Patient|Medical Record|MRN|Medical Record Number)[:\s]*([A-Z0-9]{5,15})", re.IGNORECASE
            ),
        ...
        (
            re.compile(
                r"(?:License Plate|Plate Number|Tag)[:\s]*([A-Z0-9]{1,3}[-\s]?[A-Z0-9]{1,4})", re.IGNORECASE
            ),
            "[LICENSE_PLATE]",
        ...
        (
            re.compile(
                r"(?:Corporation|Corp\.|Inc\.|LLC|LLP|Ltd\.|Company|Co\.)\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*",
                re.IGNORECASE,
            ),
            "[BUSINESS_NAME]",
        ...
        (
            re.compile(
                r"(?:doing business as|d/b/a|DBA)[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)", re.IGNORECASE
            ),
            "[DBA_NAME]",
            "DBA business names",
        ),
    ...
    def __init__(self):
        ...
        self._compiled_patterns = [
            (pattern, replacement, desc) for pattern, replacement, desc in self.PII_PATTERNS
        ]
...
__all__ = ["PIISanitizer", "sanitize_for_api", "sanitize_for_logging", "sanitize_text"]
