...
class TimelineAnalyzer:
    ...
    def __init__(self):
        ...
        self.date_patterns = [
            ...
            # Month DD, YYYY
            (r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* " r"\d{1,2},? \d{4}\b"),
    ...
    def extract_timeline_from_content(self, content: str, source: str = "unknown") -> List[Dict[str, Any]]:
        ...
        logger.info(f"TIMELINE ANALYZER: 📅 Extracted {len(events)} timeline events from {source}")
        return events
    ...
    def create_comprehensive_timeline(self, analysis_data: Any) -> Dict[str, Any]:
        ...
        entry_log = {
            ...
            "input_data": {
                ...
                "intake_summary_content": str(getattr(analysis_data, "intake_summary", "N/A"))[:500]
                if hasattr(analysis_data, "intake_summary")
                else "N/A",
                "case_documents_count": len(getattr(analysis_data, "case_documents", []))
                if hasattr(analysis_data, "case_documents")
                else 0,
                "video_insights_count": len(getattr(analysis_data, "video_insights", []))
                if hasattr(analysis_data, "video_insights")
                else 0,
            },
        }
        logger.info(json.dumps(entry_log, indent=2))
        logger.debug(f"TIMELINE ANALYZER DEBUG - ENTRY LOG: {json.dumps(entry_log, indent=2)}")
        ...
        if hasattr(analysis_data, "case_documents") and analysis_data.case_documents:
            for i, doc in enumerate(analysis_data.case_documents):
                ...
                doc_events = self.extract_timeline_from_content(doc_content, f"document_{doc_name}")
        ...
        logger.info(f"TIMELINE ANALYZER: 🗓️  Timeline created with {len(unique_events)} events")
        ...
        logger.debug(f"TIMELINE ANALYZER DEBUG - EXIT LOG: {json.dumps(exit_log, indent=2)}")
    ...
    def _parse_date_string(self, date_string: str) -> Optional[datetime]:
        ...
        try:
            ...
            logger.error(f"TIMELINE ANALYZER: ⚠️  Date parsing failed for '{date_string}': {e}")
        ...
        for event in events:
            ...
            if key not in seen_combinations:
                ...
                for existing_event in unique_events:
                    existing_date_str = (
                        existing_event["date"].strftime("%Y-%m-%d") if existing_event["date"] else "no_date"
                    )
    ...
    def _find_date_clusters(
        ...
        for _, event in enumerate(dated_events):
