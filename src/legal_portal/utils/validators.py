...
def preprocess_ai_output(raw_output: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in raw_output.items():
        ...
        if isinstance(value, str):
            ...
            # Clean list items
            processed[clean_key] = [item.strip() if isinstance(item, str) else item for item in value]
...
def safe_model_validate(
    ...
    try:
        ...
        logger.error(f"VALIDATORS: Validation failed for {model_class.__name__}: {e}")
        try:
            # Try with fallback data
            fallback_data = fallback_func()
            return model_class.model_validate(fallback_data)
        except Exception as fallback_error:
            logger.error(f"VALIDATORS: Fallback validation also failed: {fallback_error}")
            return None
...
def ensure_required_fields(data: dict[str, Any], required_fields: list[str]) -> dict[str, Any]:
    ...
    for field in required_fields:
        if field not in result or result[field] is None:
            ...
            if field.endswith("_list") or (field.endswith("s") and not field.endswith("_assessment")):
                result[field] = []
...
def validate_next_steps_formatting(content: str) -> None:
    ...
    logger.debug("VALIDATORS: Next steps formatting validation passed - <strong> tags found")
...
def validate_section_output(output_content: str, output_format: str) -> None:
    ...
    if output_format == "json":
        ...
        if not html_pattern:
            logger.error("VALIDATORS: HTML validation failed - no required HTML tags found")
            raise ValueError(
                "HTML format validation failed: required HTML tags (<p>, <ul>, <li>) are missing"
            )

        logger.info(f"VALIDATORS: HTML format validation passed for content length {len(output_content)}")

    else:
        # Handle unknown formats gracefully - log warning but don't fail validation
        logger.warning(f"VALIDATORS: Warning - Unknown output format '{output_format}', skipping validation")
