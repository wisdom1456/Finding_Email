...
class PerformanceTestSuite:
    def __init__(self):
        self.session_manager = CostSessionManager(session_storage_dir="perf_test_sessions")
    ...
    def test_large_document_estimation(self):
        ...
        for i in range(100):
            doc = ProcessedDocument(
                ...
                content="This is a sample legal document with approximately 500 words. " * 50,  # ~25k words each
        ...
        try:
            ...
            print(f"   ✅ Processed {len(large_doc_set)} documents in {processing_time:.3f}s")
            print(f"   💰 Total estimated cost: ${float(estimate.total_estimated_cost):.4f}")
            print(f"   ⏱️  Average time per document: {processing_time / len(large_doc_set):.4f}s")
    ...
    def test_concurrent_sessions(self):
        ...
        def create_session(session_id):
            ...
            try:
                ...
                return {
                    ...
                    'success': True,
                    'estimated_cost': float(summary.cost_estimate.total_estimated_cost) if summary and summary.cost_estimate else 0.0
                }
        ...
        print(f"   ✅ Created {len(successful_sessions)} concurrent sessions in {processing_time:.3f}s")
        if failed_sessions:
    ...
    def test_export_performance(self):
        ...
        actual_costs = ActualCosts(
            ...
            total_actual_cost=sum(cost.total_cost for cost in large_cost_data) * Decimal('1.1'),
        ...
        cost_summary = CostSummary(
            ...
            cost_variance=actual_costs.total_actual_cost - cost_estimate.total_estimated_cost,
        ...
        for format_type in formats:
            try:
                start_time = time.time()
                export_data = self.session_manager.export_session_budget(test_case_id, format_type)
                end_time = time.time()
                ...
                print(f"   ✅ {format_type.upper()} export: {len(export_data)} chars in {end_time - start_time:.3f}s")
    ...
    def test_memory_efficiency(self):
        ...
        try:
            ...
            self.results['memory_efficiency'] = {
                ...
                'status': 'PASSED' if memory_growth < 50 else 'WARNING'  # Warn if >50MB growth
            }
            
            print(f"   📊 Initial memory: {initial_memory:.1f} MB")
            print(f"   📈 Final memory: {final_memory:.1f} MB")
            print(f"   💾 Memory growth: {memory_growth:.1f} MB ({memory_growth/5:.1f} MB per session)")
            
        except ImportError:
            print("   ⚠️  psutil not available, skipping memory test")
            self.results['memory_efficiency'] = {'status': 'SKIPPED', 'reason': 'psutil not available'}
        except Exception as e:
    ...
    def generate_performance_report(self, total_time):
        ...
        if 'large_doc_estimation' in self.results and self.results['large_doc_estimation']['status'] == 'PASSED':
        ...
        if 'concurrent_sessions' in self.results:
            ...
            if concurrent_result['status'] in ['PASSED', 'PARTIAL']:
                success_rate = (concurrent_result['successful_sessions'] / concurrent_result['total_sessions']) * 100
