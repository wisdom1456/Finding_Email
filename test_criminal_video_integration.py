...
class TestCriminalVideoIntegration:
    ...
    def sample_criminal_intake(self):
        ...
        return EnhancedIntakeAnalysis(
            ...
            client_priorities=[
                ...
                "Suppress field sobriety test evidence",
            ],
            desired_outcomes=[
                "Case dismissal",
                "Reduced charges",
            ...
            key_facts=[
                "Traffic stop at 2:15 AM",
                "Officer claimed erratic driving",
                "Field sobriety tests conducted on uneven surface",
            ],
        )
        ...
        criminal_analysis = CriminalVideoAnalysis(
            evidence_items=[
                ...
                CriminalEvidenceItem(
                    ...
                    time_range=TimeRange(
                        start_time="04:20", end_time="09:15", confidence=0.88
                    ),
                ...
                CriminalEvidenceItem(
                    ...
                    time_range=TimeRange(
                        start_time="12:30", end_time="13:45", confidence=0.95
                    ),
                    evidence_strength="strong",
    ...
    def test_enhanced_video_insight_criminal_data_structure(
        self, mock_criminal_video_insight
    ):
        """
    ...
    def test_backward_compatibility_with_standard_video_insight(
        self, mock_standard_video_insight
    ):
        """
            ...
            criminal_analysis=CriminalVideoAnalysis(
                evidence_items=[
                    CriminalEvidenceItem(
                        ...
                        time_range=TimeRange(
                            start_time="00:15", end_time="01:45", confidence=0.9
                        ),
                        evidence_strength="strong",
    ...
    def test_video_processor_standard_case_processing(self, MockVideoProcessor):
        ...
        assert (
            not hasattr(result, "is_criminal_case") or result.is_criminal_case is False
        )
        assert not hasattr(result, "criminal_analysis")

    # ===== TEMPLATE INTEGRATION TESTS =====

    def test_findings_letter_template_criminal_integration(
        self, sample_criminal_intake, mock_criminal_video_insight
    ):
        """
        ...
        assert (
            "Driving Pattern & Reason for Stop" in rendered
            or "DRIVING_PATTERN_REASON_FOR_STOP" in rendered
        )
    ...
    def test_document_appendix_template_criminal_integration(
        self, sample_criminal_intake, mock_criminal_video_insight
    ):
        """
    ...
    def test_template_dual_mode_handling(
        self,
        sample_criminal_intake,
        sample_standard_intake,
        mock_criminal_video_insight,
        mock_standard_video_insight,
    ):
        """
    ...
    def test_email_generator_criminal_video_appendix_generation(
        self, mock_openai, sample_criminal_intake, mock_criminal_video_insight
    ):
        """
        ...
        appendix = generator._generate_video_analysis_appendix(
            analysis, persona="Test persona"
        )
    ...
    @patch("backend_logic.email_generator.EmailGenerator._make_openai_request")
    def test_email_generator_video_preservation_notice(
        self, mock_openai, sample_criminal_intake
    ):
        """
        ...
        analysis = CaseAnalysisResult(
            intake_analysis=sample_criminal_intake, video_insights=[large_video]
        )
        ...
        assert (
            "truncated due to size" in appendix.lower() or "summary" in appendix.lower()
        )
    ...
    @patch(
        "backend_logic.email_generator.EmailGenerator.generate_email_and_analysis_docs"
    )
    def test_end_to_end_criminal_video_workflow(
        self,
        mock_email_gen,
        mock_video_proc,
        sample_criminal_intake,
    ...
    def test_criminal_analysis_with_missing_evidence_categories(
        self, sample_criminal_intake
    ):
        """
            ...
            criminal_analysis=CriminalVideoAnalysis(
                evidence_items=[
                    CriminalEvidenceItem(
                        ...
                        time_range=TimeRange(
                            start_time="00:30", end_time="01:15", confidence=0.70
                        ),
    ...
    def test_mixed_criminal_and_standard_video_processing(
        ...
        assert (
            not hasattr(standard_video, "is_criminal_case")
            or standard_video.is_criminal_case is False
        )
        assert not hasattr(standard_video, "criminal_analysis")

    @patch("backend_logic.email_generator.EmailGenerator._make_openai_request")
    def test_criminal_video_analysis_error_handling(
        self, mock_openai, sample_criminal_intake
    ):
        """
        ...
        with pytest.raises(Exception):
            ...
            generator._generate_video_analysis_appendix(
                analysis, persona="Test persona"
            )
    ...
    def test_criminal_analysis_performance_with_varying_evidence_counts(
        self, evidence_count, sample_criminal_intake
    ):
        """
        ...
        for i in range(min(evidence_count, len(categories))):
            evidence_items.append(
                CriminalEvidenceItem(
                    ...
                    time_range=TimeRange(
                        start_time=f"{i:02d}:00", end_time=f"{i:02d}:30", confidence=0.8
                    ),
        ...
        criminal_analysis = CriminalVideoAnalysis(
            ...
            missing_categories=categories[evidence_count:]
            if evidence_count < len(categories)
            else [],
        )
        ...
        assert len(video.criminal_analysis.missing_categories) == max(
            0, 16 - evidence_count
        )
