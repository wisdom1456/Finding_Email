...
def main():
    ...
    # Test both results
    results = [
        ("_apply_final_cleanup", result1),
        ("global_normalization", result2)
    ]
    ...
    for method_name, result in results:
        ...
        if fix_successful:
            print(f"✓ {method_name}: CSS CORRUPTION FIX SUCCESSFUL!")
        else:
            print(f"✗ {method_name}: CSS CORRUPTION FIX FAILED")
    
    # Overall validation
    print("\n=== OVERALL FIX VALIDATION ===")
    
    all_successful = all(
        analyze_css_formatting(extract_css_from_html(result))["has_newlines"] and
        not analyze_css_formatting(extract_css_from_html(result))["is_corrupted"]
        for _, result in results
    )
    
    if all_successful:
        print("✓ ALL CSS CORRUPTION FIXES SUCCESSFUL!")
        print("  - CSS formatting preserved across all methods")
        print("  - Newlines maintained in <style> blocks")
        print("  - Multi-line CSS structure restored")
    else:
        print("⚠ MIXED RESULTS - Some fixes may need refinement")
    
    # Save the best result for manual inspection
    try:
        with open("test_data/processed_css_fixed_sample.html", "w", encoding="utf-8") as f:
            f.write(results[0][1])  # Save the _apply_final_cleanup result
        print("✓ Processed result saved to test_data/processed_css_fixed_sample.html")
    except Exception as e:
