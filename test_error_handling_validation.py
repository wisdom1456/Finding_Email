...
class ValidationResults:
    ...
    def add_test(self, test_name: str, success: bool, details: str, execution_time: float = 0):
    ...
    def test_environment_variables(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    client = OpenAI()  # Should fail with missing API key
            ...
            self.results.add_test(
                "missing_openai_key_handling",
                False, 
                f"❌ Unexpected error during environment variable test: {str(e)}",
                time.time() - start_time
            )
        ...
        for filename, content, content_type in test_files:
            ...
            try:
                ...
                self.results.add_test(
                    f"invalid_file_{filename}",
                    False,
                    f"❌ Unexpected error testing {filename}: {str(e)}",
                    time.time() - start_time
                )
    ...
    def test_direct_function_call_error_handling(self):
        ...
        try:
            ...
            try:
                ...
                result = asyncio.run(processor.process_documents_from_streamlit(None, []))
                self.results.add_test(
            ...
            try:
                analyzer = AIAnalyzer(None, processor)  # Invalid client
                self.results.add_test(
            ...
            self.results.add_test(
                ...
                False,
                f"❌ Unexpected error in direct function call test: {str(e)}",
                time.time() - start_time
            )
        ...
        try:
            ...
            try:
                ...
                if os.environ.get('OPENAI_API_KEY'):
                    client = OpenAI()
                    analyzer = AIAnalyzer(client, None)
            ...
            self.results.add_test(
                "logging_system_output",
                False,
                f"❌ Error testing logging system: {str(e)}",
                time.time() - start_time
            )
    ...
    def test_ai_service_connectivity(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    result = asyncio.run(analyzer._make_openai_request("test prompt", "gpt-4o-mini"))
                    self.results.add_test(
            ...
            self.results.add_test(
                "ai_service_connectivity",
                False,
                f"❌ Unexpected error testing AI connectivity: {str(e)}",
                time.time() - start_time
            )
