...
def test_citation_filtering():
    ...
    citations_found = any(
        re.search(pattern, result, re.IGNORECASE) for pattern in citation_patterns
    )

    if citations_found:
        logger.error(
            f"❌ Citation filtering failed - citations still present in: {result}"
        )
        return False
    logger.info("✅ Citation filtering successful")
    logger.info(f"   Original: {len(test_content)} chars")
    logger.info(f"   Filtered: {len(result)} chars")
...
def test_sentence_splitting():
    ...
    logger.info(f"   Result sentences: {len(sentences)}")
    logger.info(f"   Max sentence length: {max_sentence_length} words")
    logger.info(f"   Sample result: {result[:150]}...")

    if max_sentence_length <= 35:
        logger.info("✅ Sentence splitting successful - no sentences exceed 35 words")
        return True
    logger.error(
        f"❌ Sentence splitting failed - longest sentence: {max_sentence_length} words"
    )
    return False
...
def test_html_preservation():
    ...
    logger.info(f"   Original length: {len(test_content)} chars")
    logger.info(f"   Processed length: {len(result)} chars")
    logger.info(f"   Sample result: {result[:200]}...")

    # Check that basic processing occurred
    if len(result) < len(test_content):
        logger.info("✅ Content was processed (citations removed)")
    else:
        logger.debug("⚠️  Content length unchanged - processing may not have occurred")
    ...
    if result and result.strip():
        logger.debug("✅ Content processing successful - clean text ready for template")
        return True
    logger.error("❌ Content processing failed - empty result")
    return False
...
def test_ai_simplification_config():
    ...
    mock_config_disabled = {"simplification": {"enabled": False}}
    ...
    test_content = (
        "This is complex legal terminology that could potentially be simplified."
    )

    result = generator._apply_optional_ai_simplification(test_content)

    if result == test_content:
        logger.info("✅ AI simplification correctly disabled - content unchanged")
        return True
    logger.info("❌ AI simplification config not respected")
    return False


def run_all_tests():
    """Run all normalization pipeline tests."""
    logger.info("🧪 Testing New Normalization Pipeline Implementation")
    logger.info("=" * 60)
    ...
    if passed == total:
        logger.info(
            "🎉 All tests passed! Normalization pipeline implementation is working correctly."
        )
        return True
