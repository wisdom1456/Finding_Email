...
class PerformanceMetrics:
    ...
    def end_measurement(self, test_name: str, success: bool = True, details: str = ""):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_memory_delta = sum(m.get('memory_delta', 0) for m in self.metrics.values())
...
class PerformanceValidator:
    ...
    def test_memory_usage_document_processing(self):
        ...
        try:
            ...
            # Process documents
            result = asyncio.run(processor.process_documents_from_streamlit(mock_files, []))
            
            # Record memory after processing
            post_processing_memory = self.metrics.process.memory_info().rss / 1024 / 1024
    ...
    def test_direct_function_call_performance(self):
        ...
        try:
            ...
            for _ in range(100):
                ...
                doc_type = processor._get_document_type("test.pdf", [])
            ...
            self.metrics.end_measurement(
                test_name,
                False,
                f"❌ Direct function call test failed: {str(e)}"
            )
    ...
    def test_large_file_processing_efficiency(self):
        ...
        try:
            ...
            # Process large file
            result = asyncio.run(processor.process_documents_from_streamlit([mock_file], []))
            ...
            self.metrics.end_measurement(
                test_name,
                False,
                f"❌ Large file processing test failed: {str(e)}"
            )
    ...
    def test_concurrent_processing_performance(self):
        ...
        try:
            ...
            mock_files = [MockUploadedFile(f, f"test_{i}.txt") for i, f in enumerate(files)]
            ...
            self.metrics.end_measurement(
                test_name,
                False,
                f"❌ Concurrent processing test failed: {str(e)}"
            )
    ...
    def test_memory_leak_detection(self):
        ...
        try:
            ...
            self.metrics.end_measurement(
                test_name,
                False,
                f"❌ Memory leak detection failed: {str(e)}"
            )
    ...
    def run_all_tests(self):
        ...
        logger.info(f"Average Duration: {summary.get('average_duration', 0):.2f}s per test")
        ...
        for test_name, result in summary.get('details', {}).items():
            ...
            logger.info(f"{status} {test_name} ({duration:.2f}s, {memory_delta:+.1f}MB): {details}")
