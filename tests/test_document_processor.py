...
from backend.utils.data_models import DocumentType, FileType, ProcessedDocument
...
class TestDocumentProcessor:
    ...
    async def test_get_document_type_intake_form(self, document_processor):
        ...
        doc_type = document_processor._get_document_type(
            "intake_form.pdf", intake_filenames
        )
        assert doc_type == DocumentType.INTAKE_FORM

        doc_type = document_processor._get_document_type(
            "other_document.pdf", intake_filenames
        )
    ...
    async def test_get_document_type_case_document(self, document_processor):
        ...
        doc_type = document_processor._get_document_type(
            "contract.pdf", intake_filenames
        )
        assert doc_type == DocumentType.CASE_DOCUMENT

        doc_type = document_processor._get_document_type(
            "correspondence.eml", intake_filenames
        )
        assert doc_type == DocumentType.CASE_DOCUMENT
    ...
    async def test_process_documents_with_mocked_files(self, document_processor):
        ...
        with patch(
            "backend_logic.document_processor.PROCESSOR_MAP", mock_file_processors
        ):
            with patch("mimetypes.guess_type") as mock_mime:
                ...
                results = await document_processor.process_documents(
                    mock_files, intake_filenames
                )
        ...
        with patch("mimetypes.guess_type", return_value=(None, None)):
            with pytest.raises(DocumentProcessingError, match="No processor available"):
                await document_processor.process_documents(
                    [mock_file], intake_filenames
                )

    @pytest.mark.asyncio
    async def test_process_documents_from_streamlit_success(
        self, document_processor, sample_pdf_content
    ):
        ...
        with patch(
            "backend_logic.document_processor.PROCESSOR_MAP",
            {"application/pdf": mock_pdf_processor},
    ...
    @pytest.mark.asyncio
    async def test_process_documents_from_streamlit_file_error(
...
class TestDocumentProcessorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_process_multiple_document_types(
        self, document_processor, temp_file_path
    ):
        ...
        async def mock_processor(file_path, doc_type, filename):
            file_type = FileType.PDF if filename.endswith(".pdf") else FileType.TXT
        ...
        with patch("backend_logic.document_processor.PROCESSOR_MAP", processor_map):
            with patch("mimetypes.guess_type") as mock_mime:
                ...
                results = await document_processor.process_documents(
                    [pdf_file, txt_file], intake_filenames
                )
                assert len(results) == 2
                # Check PDF result
    ...
    async def test_error_handling_preserves_other_files(self, document_processor):
        ...
        with patch(
            "backend_logic.document_processor.PROCESSOR_MAP",
            {"application/pdf": mock_pdf_processor},
        ), patch("mimetypes.guess_type") as mock_mime:
            mock_mime.side_effect = [
                ("application/pdf", None),
