...
class SessionManager:
    ...
    def check_session_timeout(self) -> bool:
        ...
        if last_activity < timeout_threshold:
            logger.info(f"Session timeout for session: {st.session_state.get('session_id', 'unknown')[:8]}...")
            self.clear_session()
            return True
...
class SessionMonitor:
    ...
    def log_login(self, username: str, session_id: str, ip_address: str = "unknown"):
        ...
        self.audit_logger.info(
            f"LOGIN - User: {username}, Session: {session_id[:8]}..., IP: {ip_address}"
        )
    
    def log_logout(self, username: str, session_id: str):
        """Log user logout."""
        self.audit_logger.info(
            f"LOGOUT - User: {username}, Session: {session_id[:8]}..."
        )
    
    def log_timeout(self, username: str, session_id: str):
        """Log session timeout."""
        self.audit_logger.info(
            f"TIMEOUT - User: {username}, Session: {session_id[:8]}..."
        )
    
    def log_failed_login(self, username: str, ip_address: str = "unknown"):
        """Log failed login attempt."""
        self.audit_logger.warning(
            f"FAILED_LOGIN - User: {username}, IP: {ip_address}"
        )
    
    def log_permission_denied(self, username: str, resource: str):
        """Log permission denied."""
        self.audit_logger.warning(
            f"PERMISSION_DENIED - User: {username}, Resource: {resource}"
        )
    ...
    def log_user_created(self, username: str, created_by: str):
        """Log user creation."""
        self.audit_logger.info(
            f"USER_CREATED - User: {username}, Created by: {created_by}"
        )
    
    def log_user_deleted(self, username: str, deleted_by: str):
        """Log user deletion."""
        self.audit_logger.info(
            f"USER_DELETED - User: {username}, Deleted by: {deleted_by}"
        )
