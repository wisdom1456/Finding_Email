...
class ValidationTestHarness:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"❌ Failed to initialize EmailGeneratorV2: {e}")
            logger.info("⚠️  Running validation in test mode without actual generation")
            self.generator = None
            self.results: List[ValidationResult] = []
    ...
    def generate_test_email(self, test_case: TestCase) -> Tuple[str, dict]:
        ...
        if not self.generator:
            logger.info("⚠️  Using mock email for validation testing")
            return self.create_mock_email_for_testing(test_case), {
                "main_letter": self.create_mock_email_for_testing(test_case)
            }

        try:
            # Generate email using the EmailGeneratorV2 system
            email_result = self.generator.generate_email_and_analysis_docs(
                test_case.case_analysis
            )

            if not email_result:
                raise ValueError("Email generation returned None")

            logger.info(f"✅ Email generated successfully for {test_case.name}")
            ...
            logger.info("⚠️  Falling back to mock email for validation testing")
            return self.create_mock_email_for_testing(test_case), {
                "main_letter": self.create_mock_email_for_testing(test_case)
            }
    ...
    def validate_criterion_1_bridges(
        self, html_content: str, sections: Dict[str, str]
    ) -> ValidationResult:
        ...
        details = (
            f"Found {bridges_found} narrative bridges in target sections. "
            + f"Bridge examples: {'; '.join(bridge_details[:3])}"
            if bridge_details
            else "No bridges detected."
        )
    ...
    def validate_criterion_2_claims_completeness(
        ...
        claims_sections = [
            ...
            if "claim" in name or "legal" in name or "analysis" in name
        ]
        ...
        for section_name in claims_sections:
            ...
            if re.search(r"<li>|<ul>|\*\s|\d+\.\s", content) and re.search(
                r"\b(element|requirement|component)\b", content, re.IGNORECASE
            ):
            ...
            if len(content.split(".")) > 3 and re.search(
                r"\b(applies|application|analysis|evidence shows)\b",
                content,
                re.IGNORECASE,
            ):
            ...
            # Check for remedies
            if re.search(
                r"\b(remedy|remedies|damages|relief|compensation)\b",
                content,
                re.IGNORECASE,
            ):
            ...
            if re.search(
                r"\b(what this means|this means|significance|implication)\b",
                content,
                re.IGNORECASE,
            ):
                required_components["what_this_means"] = True
                evidence_details.append(
                    f"'What this means' explanation found in {section_name}"
                )
        ...
        details = (
            f"Found {components_present}/4 required components: "
    ...
    def validate_criterion_3_next_steps_completeness(
        self, sections: Dict[str, str]
    ) -> ValidationResult:
        ...
        next_steps_sections = [
            name
            for name in sections
            if "next" in name or "step" in name or "recommend" in name
        ]
        ...
        if next_steps_content:
            ...
            if re.search(
                r"\b(file|submit|gather|prepare|schedule|contact)\b",
                next_steps_content,
                re.IGNORECASE,
            ):
            ...
            for pattern in deadline_patterns:
                if re.search(pattern, next_steps_content, re.IGNORECASE):
                    ...
                    evidence_details.append(
                        f"Deadline found: {re.search(pattern, next_steps_content, re.IGNORECASE).group()}"
                    )
                    break
            ...
            for pattern in consequence_patterns:
                if re.search(pattern, next_steps_content, re.IGNORECASE):
                    ...
                    evidence_details.append(
                        "Consequences of missing deadlines described"
                    )
                    break
        ...
        details = (
            f"Found {components_present}/3 required components in next steps: "
            + ", ".join([k for k, v in required_components.items() if v])
            + f". Evidence: {'; '.join(evidence_details)}"
        )
    ...
    def validate_criterion_4_normalization_rules(
        ...
        if len(unique_sentences) == len(sentences):
            ...
            issues_found.append(
                f"Found {len(sentences) - len(unique_sentences)} duplicate sentences"
            )
        ...
        details = (
            f"Normalization score: {normalization_score}/{max_score}. "
            + f"Issues: {'; '.join(issues_found)}"
            if issues_found
            else "All normalization rules passed."
        )
    ...
    def validate_criterion_5_call_to_action(
        self, html_content: str
    ) -> ValidationResult:
        ...
        details = (
            f"Call-to-action {'found' if cta_found else 'not found'}. "
            + f"Position {'correct' if cta_position_correct else 'incorrect'}. "
            + f"Evidence: {cta_evidence[:3]}"
            if cta_evidence
            else "No CTA detected."
        )
    ...
    def validate_criterion_6_readability_score(
        self, html_content: str
    ) -> ValidationResult:
        ...
        try:
            ...
            details = (
                f"Flesch Reading Ease: {flesch_score:.1f} ({'PASS' if passed else 'FAIL'} - target ≥60). "
    ...
    def run_comprehensive_validation(self) -> Dict[str, ValidationResult]:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                logger.info(
                    f"📄 Extracted {len(sections)} sections from generated email"
                )

                # Run all validation criteria
                case_results = {
                    "criterion_1": self.validate_criterion_1_bridges(
                        email_content, sections
                    ),
                    "criterion_2": self.validate_criterion_2_claims_completeness(
                        sections
                    ),
                    "criterion_3": self.validate_criterion_3_next_steps_completeness(
                        sections
                    ),
                    "criterion_4": self.validate_criterion_4_normalization_rules(
                        email_content
                    ),
                    "criterion_5": self.validate_criterion_5_call_to_action(
                        email_content
                    ),
                    "criterion_6": self.validate_criterion_6_readability_score(
                        email_content
                    ),
                }
                ...
                validation_results[test_case.name] = {
                    ...
                    "overall_passed": all(
                        result.passed for result in case_results.values()
                    ),
                }
    ...
    def generate_validation_report(self, validation_results: Dict) -> str:
        ...
        report.append("=" * 60)
        report.append(
            f"Report Generated: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        report.append("")
        ...
        report.append(
            f"Overall System Status: {'PASS' if passed_cases == total_cases else 'FAIL'}"
        )
        report.append("")
            ...
            if "validation_results" in case_result:
                for criterion_key, result in case_result["validation_results"].items():
                    if criterion_key not in criteria_summary:
                        criteria_summary[criterion_key] = {
                            "passed": 0,
                            "total": 0,
                            "details": [],
                        }
                    ...
                    criteria_summary[criterion_key]["details"].append(
                        f"{case_name}: {result.details}"
                    )
        ...
        for case_name, case_result in validation_results.items():
            ...
            overall_status = (
                "PASS" if case_result.get("overall_passed", False) else "FAIL"
            )
        ...
        if passed_cases == total_cases:
            ...
            report.append(
                "   Review and address the following issues before deployment:"
            )

            for criterion_key, summary in criteria_summary.items():
                if summary["passed"] < summary["total"]:
                    report.append(
                        f"   • {criterion_key.replace('_', ' ').title()}: Needs attention"
                    )
...
def main():
    ...
    logger.info("🔬 Legal Letter Generation System - Comprehensive Validation")
    logger.debug("Validating 6 acceptance criteria across multiple test cases...")
    ...
    try:
        ...
        logger.info("\n📄 Full validation report saved to: validation_test_harness.py")
        ...
        logger.error(f"❌ Validation harness failed: {e}")
        return 1
